...
// For binary output of the domain data
    ...
    if ((getGeoType() == GEOLIB::GEODOMAIN) ||
        (dat_type_name.compare("BINARY") != 0))
    {
...
structures 09/2010 TF signature changed, removed some variables
**************************************************************************/
    ...
    while (!new_keyword)
    {
        ...
        if (line_string.find("$ELE_VALUES") != string::npos)
        {
            ...
            /*
               ...
               TEC_FILE_EXTENSION; remove(tec_file_name.c_str());
             */
            continue;
        }
        //--------------------------------------------------------------------
        ...
        if (line_string.find("$RWPT_VALUES") != string::npos)
        {
            while ((!new_keyword) && (!new_subkeyword))
            {
                ...
                if (name.compare("CONTROL_PLANE_NORMAL_X") == 0)
                {
                    ...
                    _control_plane_x_normal_vector.push_back(
                        control_plane_normal_position);
                }
                if (name.compare("CONTROL_PLANE_NORMAL_Y") == 0)
                {
                    in >> control_plane_normal_position;
                    _control_plane_y_normal_vector.push_back(
                        control_plane_normal_position);
                }
                if (name.compare("CONTROL_PLANE_NORMAL_Z") == 0)
                {
                    in >> control_plane_normal_position;
                    _control_plane_z_normal_vector.push_back(
                        control_plane_normal_position);
                }
        ...
        if (line_string.find("$PCS_TYPE") != string::npos)
        {
            ...
            /* // Comment by WW
               ...
               TEC_FILE_EXTENSION; remove(tec_file_name.c_str()); tec_file_name
               = pcs_type_name + "_" + "domain" + "_tri" + TEC_FILE_EXTENSION;
               remove(tec_file_name.c_str());
               tec_file_name = pcs_type_name + "_" + "domain" + "_quad" +
               TEC_FILE_EXTENSION; remove(tec_file_name.c_str()); tec_file_name
               = pcs_type_name + "_" + "domain" + "_tet" + TEC_FILE_EXTENSION;
               remove(tec_file_name.c_str());
               tec_file_name = pcs_type_name + "_" + "domain" + "_pris" +
               TEC_FILE_EXTENSION; remove(tec_file_name.c_str()); tec_file_name
               = pcs_type_name + "_" + "domain" + "_hex" + TEC_FILE_EXTENSION;
               remove(tec_file_name.c_str());
             */
            continue;
        }
 ...
 */
        ...
        if (!_ele_value_vector.empty() && _tecplot_cell_centered_element_output)
            ELECCWriteDOMDataTec(
                tec_file_name1 + "_ele" + tec_file_name2 + TEC_FILE_EXTENSION,
                te, ele_type_name);
    }
}
                                 ...
                                 string const & ele_type)
{
        ...
        //      + TEC_FILE_EXTENSION; fstream tec_file1
        //      (tec_file_name.data(),ios::app|ios::out);
        ...
        //      + TEC_FILE_EXTENSION; fstream tec_file
        //      (tec_file_name.data(),ios::app|ios::out);
        //
...
void COutput::setDataArrayDisp()
{
    ...
    MPI_Allgatherv(&size_local, 1, MPI_INT, i_recv, i_cnt, i_disp, MPI_INT,
                   MPI_COMM_WORLD);
...
void COutput::NODDomainWriteBinary_Header()
{
    ...
    file_name = file_base_name + "_" +
                convertProcessTypeToString(getProcessType()) + "_domain_" +
...
void COutput::NODDomainWriteBinary()
{
    ...
    file_name = file_base_name + "_" +
                ...
                "_domain_variables" + ".bin";
    std::cout << "Name of the binary file for node and element data: "
              << file_name << "\n";
    ...
    if (!_new_file_opened)
    {
        rc = MPI_File_open(MPI_COMM_WORLD, &file_name[0],
    ...
    MPI_File_set_view(fh, offset_new, MPI_DOUBLE, MPI_DOUBLE, &ftype[0],
                      MPI_INFO_NULL);
    ...
    for (size_t i = 0; i < num_prim_unknowns; i++)
    {
        ...
        MPI_File_set_view(fh, offset_new, MPI_DOUBLE, MPI_DOUBLE, &ftype[0],
                          MPI_INFO_NULL);
        MPI_File_write(fh, node_values, n_unknowns, MPI_DOUBLE,
    ...
    {
        double* node_values =
            m_pcs->getNodeValue_per_Variable(2 * num_prim_unknowns + i);
        offset_new = offset + int_disp * sizeof(double);
        MPI_File_set_view(fh, offset_new, MPI_DOUBLE, MPI_DOUBLE, &ftype[0],
                          MPI_INFO_NULL);
        MPI_File_write(fh, node_values, n_unknowns, MPI_DOUBLE,
        ...
        else
        {
            for (size_t k = 0; k < nName; k++)
            {
                ...
                {  // WW
                    ...
                    if (NodeIndex[k] > -1)
                    {
                        if (_nod_value_vector[k].find("DELTA") ==
                            0)  // JOD 2014-11-10
                            val_n = m_pcs->GetNodeValue(n_id, 1) -
                                    m_pcs->GetNodeValue(n_id, NodeIndex[k]);
                        else
                            val_n =
                                  ...
                                  unsigned mat_group_idx)
{
    for (size_t i = 0; i < m_msh->ele_vector.size(); i++)
    {
        ...
        // NW
        if (m_msh->ele_vector[i]->GetElementType() == e_type &&
            m_msh->ele_vector[i]->GetPatchIndex() == mat_group_idx)
            m_msh->ele_vector[i]->WriteIndex_TEC(tec_file);
    }
}
                                   ...
                                   std::string  const & ele_type_name)
{
    ...
    if (_tecplot_zones_for_mg)
    {
        ...
        {
            if (WriteELECellCenteredValuesTECHeader(tec_file, te, ele_type_name,
                                                    mg_idx, written_zones))
            {
                WriteELECellCenteredValuesTECData(tec_file, te, mg_idx,
                                                  written_zones);
                ++written_zones;
                if (!tecplot_zone_share || ! _new_file_opened)
    ...
    {
        //--------------------------------------------------------------------
        WriteELECellCenteredValuesTECHeader(tec_file, te, ele_type_name);
...
bool COutput::WriteELECellCenteredValuesTECHeader(fstream& tec_file,
                                                  int e_type,
                                                  const string& ele_type_name,
                                                  unsigned mg_idx,
                                                  unsigned written_zones)
{
    // OK411
    size_t no_elements = 0;
    const size_t mesh_ele_vector_size(m_msh->ele_vector.size());
    for (size_t i = 0; i < mesh_ele_vector_size; i++)
        if (m_msh->ele_vector[i]->GetMark() &&
            ...
            (!_tecplot_zones_for_mg ||
             m_msh->ele_vector[i]->GetPatchIndex() == mg_idx))
            no_elements++;
    ...
    if (tecplot_zone_share)
    {
        ...
        if (_new_file_opened || (_tecplot_zones_for_mg && written_zones > 1))
            tec_file << "VARSHARELIST=([1-3]=1)\n";
    }
    return true;
}
...
//
    ...
    //      "\n"; tec_file << "Warning in COutput::NODWritePLYDataTEC - no GEO
    //      data: "
    //         << geo_name << "\n";
    //      tec_file.close();
    //      return 0.0;
    ...
    // geo_name << "\n"; OKtec_file.close(); OKToDo return;
    else
    ...
    for (size_t j(0); j < nodes_vector.size(); j++)
    {
        ...
        for (size_t k = 0; k < no_variables; k++)
        {
            ...
            // does not work for multiple flow processes if (!b_specified_pcs)
            // //NW
    ...
    if (m_sfc)
    {
        ...
        for (size_t i = 0; i < nodes_vector.size(); i++)  // AB SB
        {
            ...
            for (size_t k = 0; k < _nod_value_vector.size(); k++)
            {
                ...
                if (_nod_value_vector[k].find("DELTA") == 0)  // JOD 2014-11-10
                    tec_file
                        << m_pcs->GetNodeValue(nodes_vector[i], 1) -
                               m_pcs->GetNodeValue(nodes_vector[i], nidx - 1)
                        << " ";
                else
...
CGLPolyline, CFEMesh and CRFProcess (conflicting with class attributes that have
the same name)
**************************************************************************/
...
void COutput::ELEWriteSFC_TECData(fstream& tec_file)
{
    ...
    /* // Make it as comment to avoid compilation warnings. 18.082011 WW
       ...
       double v_n;
       //--------------------------------------------------------------------
       m_pcs = pcs_vector[0]; //GetPCS_ELE(ele_value_vector[0]); int nidx[3];
...
void COutput::ELEWritePLY_TECData(fstream& tec_file)
{
    ...
    for (size_t i = 0; i < ele_vector_at_geo.size(); i++)
    {
        ...
        // tec_file << " " << pcs->GetElementValue(m_ele->GetIndex(),
        // f_eidx[0]); tec_file << " " <<
        // pcs->GetElementValue(m_ele->GetIndex(), f_eidx[1]); tec_file << " "
        // << pcs->GetElementValue(m_ele->GetIndex(), f_eidx[2]);
        tec_file << "\n";
    }
}
...
void COutput::addInfoToFileName(std::string& file_name, bool geo, bool process,
                                bool mesh, const std::string& ext) const
{
    // add geo type name
    if (geo)
...
void COutput::CalculateTotalFlux(CFEMesh* msh, vector<long>& nodes_on_geo,
                                 vector<double>& node_value_vector_diff,
...
void COutput::NODWritePrimaryVariableList(double time_current)
{
    ...
    if (geo_name.size() == 0)
        geo_name = "domain";
    string tec_file_name = convertProcessTypeToString(getProcessType()) + "_" +
                           geo_name + "_primary_variables.txt";
    ...
    for (size_t j = 0; j < time_vector.size(); j++)
        if ((fabs(time_current - time_vector[j])) <
                    ...
                    for (std::size_t i = 0; i < m_msh->nod_vector.size(); i++)
                        tec_file << m_msh->nod_vector[i]->GetIndex()
                                 ...
                                 << m_pcs_out->GetNodeValue(
                                        m_msh->nod_vector[i]->GetIndex(), 1)
                                 << "\n";

                    cout << "Data output: "
                         << convertProcessTypeToString(getProcessType())
                         << " primary variables - DOMAIN - "
                         << m_msh->nod_vector.size() << " nodes" << endl;
                    break;
                case GEOLIB::SURFACE:
...
void COutput::NODWriteTotalFlux(double time_current, int time_step_number)
{
    ...
    CalculateTotalFlux(m_msh, nodes_vector, nodes_value_vector_diffusion,
                       nodes_value_vector_advection);

    total_value_diffusion = total_value_advection = 0;
    for (long i = 0; i < (long)nodes_value_vector_diffusion.size(); i++)
    {
        total_value_diffusion += nodes_value_vector_diffusion
            [i];  // fabs(nodes_value_vector_diffusion[i]);
        total_value_advection += nodes_value_vector_advection
            [i];  // fabs(nodes_value_vector_advection[i]);
    }
    ...
    cout << "Data output: " << convertProcessTypeToString(getProcessType())
         << " TOTAL_FLUX " << geo_name << " -  " << nodes_vector.size()
         << " nodes" << endl;
    tec_file.close();
}
...
void COutput::SetTotalFluxNodes(std::vector<long>& nodes_vector)
{
    switch (this->getGeoType())
    {
        ...
        case GEOLIB::GEODOMAIN:
            ...
            break;
        default:
            cout << "Warning: Water Balance does not support this geotype"
                 << endl;
    }
}
