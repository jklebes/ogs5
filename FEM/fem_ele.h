...
public:
    ...
    void setShapeFunctionPool(
        const ShapeFunctionPool* const lin_shape_fct_pool,
        const ShapeFunctionPool* const quad_shape_fct_pool)
    {
        _shape_function_pool_ptr[0] = lin_shape_fct_pool;
        _shape_function_pool_ptr[1] = quad_shape_fct_pool;
    };

    const ShapeFunctionPool* getShapeFunctionPool(int order_id) const
    {
    ...
    // Compute values of the derivatives of shape function at integral point
    void computeGradShapefctLocal(const int order,
...
protected:
    ...
    size_t dim;      // Dimension of the space
    // Dimension of the space where an element exists.
    size_t _ele_global_dim;
    ...
    /// The values of derivatives of linear shape functions all integation
    /// points.
    double* _dshapefct_all;
    /// The values of derivatives of quadratic shape functions all integation
    /// points.
    ...
    // Get the values of the local gradient of shape functions at integral point
    // gp
...
private:
    // TODO in more gernal way for the case of sym and unsym. WW      SymMatrix
    // *Mass;
    //      SymMatrix *Laplace;
    Matrix* Mass;
    Matrix* Mass_notsym;
