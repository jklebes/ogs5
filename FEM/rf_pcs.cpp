...
CRFProcess::CRFProcess(void)
    : _problem(NULL),
      ...
      _init_domain_data_type(FiniteElement::NO_IO),
#ifdef JFNK_H2M
      JFNK_precond(false),
      norm_u_JFNK(NULL),
      array_u_JFNK(NULL),
...
//
    ...
    PrecalculatedFiles = false;    // BG, 01/2011, flag for using already
                                   // calculated files from Eclipse or DuMux
    SaveEclipseDataFiles = false;  // Standard case: do not save files
    Phase_Transition_Model = 0;    // BG, 11/2010, flag for using CO2 Phase
                                   // transition (0...not used, 1...used)
    // SB redo wtp
    dissolved_co2_pcs_name =
        "";  // SB, CB 10/2011 Name of MASS_TRANSPORT process used to store
             // dissolved total CO2 in water
...
CRFProcess::~CRFProcess(void)
{
    ...
    PetscPrintf(
        PETSC_COMM_WORLD,
        "\t\n>>Total Wall clock time in the assembly for %s (with PETSC):%f "
        "s\n",
        FiniteElement::convertProcessTypeToString(this->getProcessType())
            .c_str(),
        cpu_time_assembly);
...
// eqs_new = EQS_Vector[2*k];
    ...
    if (!Tim)
    {
        ...
        the properties of some specified [0] vector. Why not set default values,
        and then let other "Tim" control the stepping? In other words. If
        HEAT_TRANSPORT doesn't have time control, we cannot assign a time
        control type for a FLOW process to a HEAT process, this could give
        incorrect results. THE DEFAULTS ARE NOW SET UP SUCH THAT... if "Tim"
        doesn't exist, this process has no influence on the time step. Tim = new
        CTimeDiscretization(*time_vector[0], pcs_type_name);
        */
    ...
    for (std::size_t i = 0; i < pcs_vector.size(); i++)
    {
        ...
        for (std::size_t j = 0; j < pcs_vector[i]->bc_node.size(); j++)
        {
            if (found)
                break;
            for (std::size_t k = 0;
                 k < pcs_vector[i]->bc_node[j]->getNumberOfConstrainedBCs();
                 k++)
            {
                if (found)
                    break;
                ...
                if (tmp.constrainedVariable == ConstrainedVariable::VELOCITY)
                {
                    ...
                    pcs_vector[i]->setidxVx(
                        pcs->GetNodeValueIndex("VELOCITY_X1", true));
                    pcs_vector[i]->setidxVy(
                        pcs->GetNodeValueIndex("VELOCITY_Y1", true));
                    pcs_vector[i]->setidxVz(
                        pcs->GetNodeValueIndex("VELOCITY_Z1", true));
        ...
        else
        {
            for (int i = 0; i < DOF; i++)
            {
                ...
                const double value_offset =
                    (primary_name.compare("TEMPERATURE1") == 0 &&
...
std::ios::pos_type CRFProcess::Read(std::ifstream* pcs_file)
{
    ...
    while (!new_keyword)
    {
            ...
            while ((!new_keyword) || (!new_subkeyword) || (!pcs_file->eof()))
            {
                ...
                if (isFlowProcess(this->getProcessType()))
                {
                    ...
                    if (this->getProcessType() == FiniteElement::PS_GLOBAL ||
                        ...
                        // this->getProcessType() == FiniteElement::TNEQ || no
                        // multiphase flow
                ...
                if (this->getProcessType() ==
                    FiniteElement::MULTI_COMPONENTIAL_FLOW)
                {
        ...
        if (line_string.find("$TEMPERATURE_UNIT") != string::npos)
        {
            ...
            *pcs_file >> T_unit_name;
            ...
            _temp_unit = (T_unit_name.find("CELSIUS") != std::string::npos)
                             ? FiniteElement::CELSIUS
                             : FiniteElement::KELVIN;
            continue;
        }
        ...
        if (line_string.find("$PROCESSED_BC") !=
            string::npos)  // 25.08.2011. WW
        {
            ...
            switch (_pcs_constant_model)
            {
                ...
                case 2:
                    *pcs_file >> _pcs_constant_curve;
                    break;
                default:
                    std::cout << "-> invalid CONSTANT model "
                              << _pcs_constant_model << std::endl;
                    break;
            }
...
void CRFProcess::Config(void)
{
    ...
    if (this->getProcessType() ==
        FiniteElement::MULTI_COMPONENTIAL_FLOW)  // 24.02.2007 WW
    {
        type = 1111;
...
void CRFProcess::ConfigMassTransport()
{
    ...
    /*
       ...
       pcs_primary_function_name[0]; pcs_ic_name_mass =
       pcs_primary_function_name[0];
     */
...
//#define DECOVALEX
        ...
        pcs_secondary_function_timelevel[pcs_number_of_secondary_nvals] = 1;
        pcs_number_of_secondary_nvals++;
        // for calibration (unconfined groundwater flow with variable
        // density) 5.3.07 JOD
...
void CRFProcess::Def_Variable_MultiPhaseFlow()
{
    ...
    if (simulator.compare("ECLIPSE") == 0)  // 02.2013. WW
    {
        ...
        // "TEMPERATURE1";
        // pcs_secondary_function_unit[pcs_number_of_secondary_nvals]      =
        // "K"; pcs_secondary_function_timelevel[pcs_number_of_secondary_nvals]
        // = 1; pcs_number_of_secondary_nvals++;
    }
}
...
void CRFProcess::ConfigTNEQ()
{
    ...
    long group_count = 0;
    m_rho_s_0 =
        msp_vector[group_count]->Density();  // get initial solid density
    double poro = mmp_vector[group_count]->porosity;
    FiniteElement::SolidReactiveSystem react_syst =
        msp_vector[group_count]->getSolidReactiveSystem();
    m_conversion_rate =
        new conversion_rate(573.0,       // T_solid, Kelvin
                            573.0,       // T_gas, Kelvin
                            0.0,         // p_gas
...
bool CRFProcess::isPointInExcavatedDomain(double const* point,
                                          double& max_excavation_range,
                                          double& min_excavation_range)
{
    ...
    const double element_center_x_in_excavation_direction =
...
/*************************************************************************
 ...
 stuff 12/2007 WW Classes of sparse matrix (jagged diagonal storage) and linear
 solver and parellelisation of them 07/2011 WW Add Newton-Rahson and reduce
 #ifdef 3/2012  JT Clean, and correct the error obtainment for NLS and CPL last
 modified:
 **************************************************************************/
...
// Execute linear solver
    ...
    if (m_num->fct_method > 0)  // NW
    {
        ...
        ScreenMessage("    Start FCT calculation\n");
...
//		exit(1);
    ...
    if (m_num->nls_method_name.find("PICARD") != string::npos)
    {
                                      ...
                                      // lesser calculations.
        {
            for (int ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (j = 0; j < g_nnodes; j++)
                {
                    ...
                    k = pcs_number_of_primary_nvals *
                            m_msh->Eqs2Global_NodeIndex[j] +
                        ii;
                    const double val_n =
                        GetNodeValue(j, nidx1);  // 03.04.2009. WW
                    SetNodeValue(j, nidx1, eqs_x[k]);
        ...
        else
        {
            for (int ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (j = 0; j < g_nnodes; j++)
                {
                    ...
                    SetNodeValue(
                        j, nidx1,
                        (1.0 - nl_theta) * val_n + nl_theta * eqs_x[k]);
                    eqs_x[k] = val_n;  // Used for time stepping. 03.04.2009. WW
#else
                    k = m_msh->Eqs2Global_NodeIndex[j];
                    const double val_n =
                        GetNodeValue(k, nidx1);  // 03.04.2009. WW
                    SetNodeValue(k, nidx1,
                                 (1.0 - nl_theta) * val_n +
        ...
        // do I really need to check every single bc node, or how can I access a
        // bc group?
...
void CRFProcess::CalIntegrationPointValue()
{
        ...
        for (size_t i = 0; i < mesh_ele_vector_size; i++)
        {
            ...
            if (elem->GetMark())  // Marked for use
            {
                if ((getProcessType() == FiniteElement::HEAT_TRANSPORT ||
                    ...
                    !elem->selected)
                    continue;  // not selected for TOTAL_FLUX calculation JOD
                               // 2014-11-10
                ...
                if (getProcessType() == FiniteElement::TNEQ ||
                    getProcessType() == FiniteElement::TES)
                {
                    fem->CalcSolidDensityRate();  // HS, thermal storage
                                                  // reactions
                }
            }
        }
    ...
    {  // NW
        ...
        double pre_v[3] = {};
        double new_v[3] = {};
        // std::cout << "  Start local Picard iteration: tolerance = " <<
        // this->m_num->local_picard1_tolerance << std::endl;
        size_t i_itr = 0;
        double vel_error = .0;
        for (i_itr = 0; i_itr < v_itr_max; ++i_itr)
        {
            // std::cout << "  non-linear iteration: " << i_itr << "/" <<
            // v_itr_max << std::endl;
            vel_error = .0;
            for (size_t i = 0; i < mesh_ele_vector_size; i++)
            {
            ...
            bool isConverged =
                (vel_error < this->m_num->local_picard1_tolerance);
            if (isConverged)
                break;
        }
        std::cout << "  Local Picard iteration: itr. count = " << i_itr << "/"
                  << v_itr_max << ", error(max. norm)=" << vel_error
                  << std::endl;
    }

    // if (getProcessType() == FiniteElement::TNEQ || getProcessType() ==
    // FiniteElement::TEQ) {
    //    const size_t mesh_ele_vector_size(m_msh->ele_vector.size());
    //    for (size_t i = 0; i < mesh_ele_vector_size; i++)
    //    {
    ...
    if (getProcessType() == FiniteElement::TWO_PHASE_FLOW ||
        getProcessType() == FiniteElement::MASS_TRANSPORT ||
        getProcessType() ==
            FiniteElement::HEAT_TRANSPORT)  // WW/CB/JOD 2014-11-10
        cal_integration_point_value = false;
}
    ...
    for (i = 0; i < Size; i++)
    {
        ...
        if (node_connected_doms[l_index] > 0)
            st_node_value[i]->node_value /=
                (double)node_connected_doms[l_index];
    }
}
...
//
        ...
        const long id_act_h_max =
            static_cast<long>(m_msh->getLargestActiveNodeID_Quadratic());
        // If ghost node
        ...
        else
        {
            ...
            {
                shift =
                    m_bc_node->msh_node_number / m_msh->NodesNumber_Quadratic;
            }
            else
            {
                shift = m_bc_node->msh_node_number / m_msh->NodesNumber_Linear;
        ...
        if (bc_msh_node >= 0)
        {
            ...
            if (m_bc_node->bc_node_copy_geom.length() > 0)
            {
                ...
                // (std::string)this->m_msh->_geo_name;
                GEOLIB::GEOObjects* geo_obj;
                geo_obj = this->m_msh->getGEOObjects();
                // Get geometry node
                const std::vector<GEOLIB::Point*>* pnt_vec(
                    geo_obj->getPointVec(*this->m_msh->getProjectName()));
                // const GEOLIB::PointVec*
                // pnt_vec(geo_obj->getPointVecObj(m_bc_node->bc_node_copy_geom));
                // const GEOLIB::Point*
                // pnt(pnt_vec->getElementIDByName(m_bc_node->bc_node_copy_geom,
                // msh_node_id));
                if (!((geo_obj->getPointVecObj(*this->m_msh->getProjectName()))
                          ->getElementIDByName(
                              m_bc_node->bc_node_copy_geom_name, geo_node_id)))
                {
                    cout << " Error !"
                         << "\n";
                }
                ...
                std::cout << " Copy value " << time_fac << " from "
                          << m_bc_node->bc_node_copy_geom << " "
                          << m_bc_node->bc_node_copy_geom_name
                          << " to POLYLINE "
                          << "\n";
            }
            //................................................................
            ...
            {
                if (m_bc->getPressureAsHeadModel() ==
                    -1)  // this is the default case
                {
                ...
                {
                    bc_value = calcPressureFromHead(
                        *m_bc, m_bc_node->msh_node_number,
                        m_bc_node->node_value, time_fac, fac);
                }
            }
            if (m_bc->isSwitchBC())
            {
...
//----------------------------------------------------------------
            ...
            if (m_num->nls_method >=
                    1  // 04.08.2010. WW _name.find("NEWTON")!=string::npos
                || type == 4 || type / 10 == 4)
            ...
            else
            {
                ...
                /* // Make the follows as comment by WW. 04.03.2008
                   ...
                   //DOF>1 WW bc_eqs_index += fem->NodeShift[ii];   //DOF>1 WW
                ...
                if (m_bc->getExcav() > 0)
                {
                    ...
                    else if (m_bc_node->pcs_pv_name.find("DISPLACEMENT") !=
                             string::npos)
                        continue;  //
                }
#endif  // end defined(USE_PETSC) // || defined(other parallel
        // libs)//03~04.3012. WW
    ...
    // We have do the following collection because MatZeroR must be called by
    // all processes
    ...
    // Get nbc
    MPI_Allgatherv(&nbc, 1, MPI_INT, &r_vec[0], &r_cnt[0], &r_disp[0], MPI_INT,
                   PETSC_COMM_WORLD);
    int v_disp = 0;
    for (k = 0; k < mpi_size; k++)
    {
...
//
            ...
            if (bc_msh_node >= 0)
            {
                ...
                // NEWTON WW
                if (m_num->nls_method >=
                        1  //_name.find("NEWTON")!=std::string::npos
                    || type == 4 ||
                    type == 41)  // Solution is in the manner of increment !
                {
                    idx0 =
...
// TODO
    ...
    /*
       ...
       == -4) { // irreg.Knoten if (GetRFControlGridAdapt()) if
       (AdaptGetMethodIrrNodes() == 1) { MXSet(i, i, MKleinsteZahl); rechts[i] =
       0.0;
          }
        }
       }
                                    ...
                                    CNodeValue const& st_node)
{
    ...
    for (std::size_t i = 0; i < st.getNumberOfConstrainedSTs(); i++)
    {
        ...
        if (local_constrained.constrainedPrimVar == FiniteElement::PRESSURE ||
            local_constrained.constrainedPrimVar ==
                FiniteElement::CONCENTRATION ||
            local_constrained.constrainedPrimVar == FiniteElement::TEMPERATURE)
        {
            ...
            for (std::size_t k = 0; k < pcs->GetPrimaryVNumber(); k++)
            {
                if (pcs->GetPrimaryVName(k) ==
                    FiniteElement::convertPrimaryVariableToString(
                        local_constrained.constrainedPrimVar))
                {
                    // value of PrimVar of other process at current node
                    double local_value =
                        pcs->GetNodeValue(st_node.geo_node_number, 2 * k + 1);

                    // FIXME: if getPressureAsHead != -1, then the head is
                    // expected however, getPressureAsHead is part of
                    // CBoundaryCondition (m_bc in
                    // IncorporateBoundaryConditions)

                    if (local_constrained.constrainedDirection ==
                        ConstrainedType::GREATER)  // exclude greater and equal
                                                   // values
                    {
                        if (local_value >=
                            local_constrained
                                .constrainedValue)  // check if calculated value
                                                    // (eg of other process)
                                                    // meets criterium
                            constrained_bool = true;
                    }
                    else if (local_constrained.constrainedDirection ==
                             ConstrainedType::SMALLER)  // exclude smaller
                                                        // values
                    {
                                    ...
                                    double& bc_value)
{
    for (std::size_t i = 0; i < bc.getNumberOfConstrainedBCs(); i++)
    {
            ...
            ConstrainedVariable::VELOCITY)
        {
            ...
            if (local_constrained._isConstrainedVelStable)
            {
                ...
                for (std::size_t j = 0; j < no_connected_nodes; j++)
                {
                    ...
                    if (connected_node_id ==
                        static_cast<std::size_t>(bc_node.geo_node_number))
                    {
                        std::valarray<double> temp_vel(
                            this->getNodeVelocityVector(connected_node_id));
            ...
            {
#ifdef DEBUG
                std::cout << "No constrained applied at node "
                          << bc_node.msh_node_number
                          << " as magnitude of velocity " << magn_vel_v
                          << " is not > than 0 " << std::endl;
#endif
                continue;
            }

            // NormalizeVector(&vel[0], 3); can use other way to calc normalized
            // vector, as vector now valarray:
            vel /= magn_vel_v;

            // calculate scalar product of velocity vector and BC surface normal
            double const scalar_prod(
                MathLib::scpr(&(vel)[0], bc_node.GetNormalVector(), 3));

            // select case to handle BC
            if (scalar_prod < 0.05 &&
                scalar_prod >= -1.01  // small tolerance of ~5°
                && local_constrained.constrainedDirection ==
                       ConstrainedType::NEGATIVE)  // velocity vector and bc
                                                   // surface normal point in
                                                   // opposite directions
            {
                return true;  // do not apply BC (maybe later implementation:
                              // change BC to ST at this node)
            }
            else if (scalar_prod <= 1.01 &&
                     scalar_prod >= -0.05  // small tolerance of ~5°
                     &&
                     local_constrained.constrainedDirection ==
                         ConstrainedType::POSITIVE)  // velocity vector points
            ...
            for (std::size_t k = 0; k < pcs->GetPrimaryVNumber(); k++)
            {
                if (pcs->GetPrimaryVName(k) ==
                    FiniteElement::convertPrimaryVariableToString(
                        local_constrained.constrainedPrimVar))
                {
                    // TODO: check case GREATER - maybe not thoroughly enough
                    // evaluated
                    if (local_constrained.constrainedDirection ==
                        ConstrainedType::GREATER)  // exclude greater and equal
                                                   // values
                    {
                        // value of PrimVar of other process at current node
                        double local_value(
                            0.1 * pcs->GetNodeValue(bc_node.geo_node_number,
                                                    2 * k + 1) +
                            0.9 * pcs->GetNodeValue(bc_node.geo_node_number,
                                                    2 * k));
                        if (local_value >=
                            local_constrained
                                .constrainedValue)  // check if calculated value
                        ...
                        if (bc.getProcessType() ==
                                local_constrained.constrainedProcessType &&
                            bc_node.node_value_pre_calc >=
                                local_constrained
                                    .constrainedValue)  // check if BC value
                                                        // meets criterium
                            return true;
                    }
                    else if (local_constrained.constrainedDirection ==
                                                        ...
                                                        // values
                    {
                        if (bc.getProcessType() ==
                            local_constrained
                                .constrainedProcessType)  // check if correct
                                                          // process type
                        {
                            if (bc_node.node_value_pre_calc <
                                local_constrained
                                    .constrainedValue)  // limit BC if
                                                        // constrained below BC
                                                        // value
                            {
                                if (local_constrained._isSeepageBC)
                                {
                                    // value of PrimVar of other process at
                                    // current node
                                    double local_value(
                                        0.1 * pcs->GetNodeValue(
                                                  bc_node.geo_node_number,
                                                  2 * k +
                                                      1)  // slight averaging
                                                          // with last time step
                                                          // for stability
                                        + 0.9 * pcs->GetNodeValue(
                                                    bc_node.geo_node_number,
                                                    2 * k));

                                    if (local_value <
                                        local_constrained.constrainedValue)
                                        return true;
                                    else
                                        bc_value =
                                            local_constrained.constrainedValue;
    ...
    std::cout << "Non existing combination for constrained BC direction given. "
                 "Using normal BC."
              << std::endl;
    return false;
}
...
int CRFProcess::getFirstNodeBelowGWL(size_t current_node)
{
    ...
    for (size_t k(1); k < this->m_msh->xy_change.size();
         k++)  // look for range of xy-coordinates
    {
        const long node_id = this->m_msh->xy_change[k];
        if (node_id >= sorted_node)  // found range
        {
            new_GWL_st_node =
                this->m_msh->sorted_nodes[this->m_msh->xy_change[k - 1] +
                                          1];  // set to bottom node first

            if (this->GetNodeValue(this->m_msh->sorted_nodes[node_id],
                                   val_idx) > 0)  // top node has p>0
            {
                new_GWL_st_node =
                    this->m_msh->sorted_nodes[node_id];  // return top node id
                break;
            }
            else
            {
                for (size_t j(node_id - 1);
                     j > this->m_msh->xy_change[k - 1] + 1;
                     j--)  // look in range from top to bottom (but skip top
                           // node)
                {
                    if (this->GetNodeValue(this->m_msh->sorted_nodes[j],
                                           val_idx) >
                        0)  // look for first node with p>0
                    {
                        new_GWL_st_node =
                            this->m_msh
                                ->sorted_nodes[j -
                                               1];  // for p<0, return node id
                                                    // two nodes below gwl
                        break;
                    }
                }
...
// MW if set, calculate pressure from given head at bc with density, gravity
// constant and geodetic height for PRESSURE as primary variable
double CRFProcess::calcPressureFromHead(CBoundaryCondition const& bc,
                                        std::size_t node_number,
                                        double head_value,
                                        double const& time_fac,
                                        double const& fac)
{
    ...
    const double local_node_elevation =
        this->m_msh->nod_vector[node_number]->Z();

    // pressure = gravitational_constant * density * ( head - geodetic_height )
    double pressure_value(fac * gravity_constant * local_density *
                          (time_fac * head_value - local_node_elevation));

    return pressure_value;
}

double CRFProcess::calcHeadFromPressure(CBoundaryCondition const& bc,
                                        std::size_t node_number,
                                        ...
                                        double const& fac)
{
    ...
    // pressure = gravitational_constant * density * ( head - geodetic_height )
    double head_value(
        fac * (pressure_value * time_fac / (gravity_constant * local_density) +
               local_node_elevation));

    return head_value;
}

double CRFProcess::evaluteSwitchBC(CBoundaryCondition const& bc,
                                   CBoundaryConditionNode const& bc_node,
                                   double time_fac, double fac)
{
    ...
    {
        if (pcs->GetPrimaryVName(i) ==
            FiniteElement::convertPrimaryVariableToString(
                localSwitchBC.switchPrimVar))
        {
            // value of PrimVar of other process at current node
            double localValue(
                0.1 * pcs->GetNodeValue(bc_node.geo_node_number, 2 * i + 1) +
...
void CRFProcess::IncorporateSourceTerms(const int rank)
{
    ...
    for (size_t i = 0; i < st_vector.size(); i++)
    {
        ...
        for (std::size_t j(0); j < st_vector[i]->getNumberOfConstrainedSTs();
             j++)
        {
        ...
        // NOTE (KR): This only works correctly if there is only ONE source term
        // with DisType CLIMATE! TODO If more are needed pls let me know
        if (st_vector[i]->getProcessDistributionType() ==
            FiniteElement::CLIMATE)
        {
            m_st = st_vector[i];
            const MathLib::InverseDistanceInterpolation<GEOLIB::PointWithID*,
                                                        GEOLIB::Station*>*
                distances(m_st->getClimateStationDistanceInterpolation());

            // Interpolate each ST to every surface node;
            const std::vector<GEOLIB::Station*>& weather_stations(
                m_st->getClimateStations());
            ...
            for (size_t j = 0; j < nStations; j++)
                vec_etr[j] = static_cast<GEOLIB::Station*>(weather_stations[j])
                                 ->getSensorData()
                                 ->getData(SensorDataType::RECHARGE,
                                           aktuelle_zeit, true);
            ...
            {
                // search the first node value index not set (this should be the
                // start for the CLIMATE data
                if (st_node_value[j]->node_value ==
                    std::numeric_limits<double>::min())
                {
                    // Interpolate for each surface node
                    for (size_t w = 0; w < nDistances; w++)
                    {
                        // for each ST_node, the distance to all weather
                        // stations has been determined before
                        // (InverseDistranceInterpolation)
                        // Value ETR(n) at node n by inverse distance weighting
                        // of ETR(WS) for each weather station WS
                        double st_value(0);
                        for (size_t q = 0; q < nStations; q++)
                            st_value += vec_etr[q] *
                                        distances->getDistance(w, q) /
                                        distances->getSumOfDistances(w);

                        const size_t n(distances->getDomainPoint(w)->getID());
                        st_node_value[j + w]->node_value =
                            st_value * m_msh->nod_vector[n]->patch_area;
                    }
                    break;
                }
            }
    ...
    for (i = begin; i < end; i++)
    {
        ...
        // MW Recharge applied at GW Surface (p>0), tested only with LIQUID_FLOW
        // or RICHARDS_FLOW and mmp $PERMEABILITY_SATURATION = 10
        if (cnodev->getProcessDistributionType() == FiniteElement::RECHARGE ||
            cnodev->getProcessDistributionType() ==
                FiniteElement::RECHARGE_DIRECT)
        {
            cnodev->msh_node_number =
                getFirstNodeBelowGWL(cnodev->msh_node_number);
            ...
            if (m_st->isConstrainedST())
            {
                     ...
                     temp_i++)
                {
                    if (st_vector[cnodev->getSTVectorGroup()]
                        ...
                        st_vector[cnodev->getSTVectorGroup()]
                            ->getCompleteConstrainedSTStateOff(temp_i))
                    {
                        continue_bool = true;
                        break;
                    }
        ...
        if (curve > 0)
        {
            ...
            if (m_st != NULL)  // in some cases the m_st is not defined ->
                               // interp_method is not changed for this cases
            ...
            // this->Tim->time_end << " Faktor: " << time_fac << "\n";
            if (!valid)
            {
                cout << "\n!!! Time dependent curve is not found. Results are "
                        "not guaranteed "
                     << "\n";
...
// EQS->RHS
    ...
    // if coupling to GEMS
    // exist---------------------------------------------------- HS,
    // added 11.2008 KG44 03/03/2010 modified to hopefully soon work with
    // parallel solvers
...
/*************************************************************************
 ...
 //SB:todo Programming:	08/2003 SB Implementation last modified: superseded by
 GetPFNamebyCPName() but left here, as not all files are already in the new
 concept
 **************************************************************************/
/* SB: namepatch
...
Referenz) method            : Methode der Fehlerermittlung Programing: 01/2005
OK NUM implementation 05/2005 OK MSH 08/2005 WW Re-implememtation based on
NUMCalcIterationError 01/2007 WW For DOF>1 11/2007 WW Changes for the new
classes of sparse and linear solver 3/2012  JT Clean, add newton, add CPL vs.
NLS, go to enum system last modification:
**************************************************************************/
                                         ...
                                         bool nls_error, bool cpl_error)
{
    ...
    switch (method)
    {
                ...
                for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
                {
                    ...
                    for (i = 0; i < g_nnodes; i++)
                    {
                        ...
                        val1 = GetNodeValue(i, nidx1) -
                               eqs_x[pcs_number_of_primary_nvals *
                                         m_msh->Eqs2Global_NodeIndex[i] +
                                     ii];
#else
        ...
        //     (relative error). A single tolerance applied to all primary
        //     variables.
        //
...
#if defined(USE_PETSC)  // || defined(other parallel libs)//08.2014. WW
                        val1 = GetNodeValue(i, nidx1);
                        val2 = val1 - eqs_x[pcs_number_of_primary_nvals *
                                                m_msh->Eqs2Global_NodeIndex[i] +
                                            ii];
#else
                            k = m_msh->Eqs2Global_NodeIndex[i];
        ...
        //     (absolute error). Tolerance required for each primary variable.
        //
        ...
        default:
            ...
            GetNodeValue(k, nidx1))); max_c = MMax(MMax(max_c,
            ...
            1;
                    ...
                    for (i = 0l; i < g_nnodes; i++){
                       k = m_msh->Eqs2Global_NodeIndex[i];
                       error = MMax(error, fabs(eqs_x[i+ii*g_nnodes] -
            GetNodeValue(k, nidx1))); min_c = MMin(min_c,
            fabs(eqs_x[i+ii*g_nnodes])); max_c = MMax(max_c,
            fabs(eqs_x[i+ii*g_nnodes]));
                    }
            ...
            1;
                    ...
                    for (i = 0l; i < g_nnodes; i++) {
                       k = m_msh->Eqs2Global_NodeIndex[i];
                       error = MMax(error, fabs(eqs_x[i+ii*g_nnodes] -
            GetNodeValue(k, nidx1)) / (fabs(eqs_x[i+ii*g_nnodes] -
            GetNodeValue(k, nidx1-1)) + MKleinsteZahl));
                    }
            ...
            1;
                    ...
                    for (i = 0l; i < g_nnodes; i++){
                       k = m_msh->Eqs2Global_NodeIndex[i];
                       error = MMax(error, fabs(eqs_x[i+ii*g_nnodes] -
            GetNodeValue(k, nidx1))); change = MMax(change,
            fabs(eqs_x[i+ii*g_nnodes] - GetNodeValue(k, nidx1-1)));
                    }
    ...
    for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
    {
        ...
        MPI_Allreduce(&val_temp, &pcs_absolute_error[ii], 1, MPI_DOUBLE,
    ...
    {  // Return coupling error
        ...
        for (ii = 0; ii < num_dof_errors; ii++)
        {
            ...
            error_g =
                MMax(error_g, error);  // Coupling error just stores the maximum
        }
    ...
    {  // Then this routine called from somewhere else to get the error (i.e.
       // time control). Store it in a temporary
        // vector for access.
        for (ii = 0; ii < num_dof_errors; ii++)
        {
...
double CRFProcess::ExecuteNonLinear(int loop_process_number, bool print_pcs)
{
    ...
    double error_x1, error_x2, error_b1, error_b2 = 0, error, last_error,
                                         percent_difference;
    // double* eqs_x = NULL;     //
            ...
            switch (m_num->getNonLinearErrorMethod())
            {
                ...
                default:
                    ...
                    {  // Check for stagnation
                        percent_difference =
                            100.0 * ((last_error - nonlinear_iteration_error) /
                                     last_error);
                        if (iter_nlin > 0 &&
                            percent_difference <
...
//
        ...
        if (m_num->nls_max_iterations > 1 && !converged &&
            iter_nlin + 1 == m_num->nls_max_iterations)
            accepted = false;
        ...
        if (m_num->nls_method > 0)
        {
            ...
            for (ii = 0; ii < pcs_number_of_primary_nvals; ii++)
            {
                ...
                for (j = 0; j < g_nnodes; j++)
                {
                    k = m_msh->Eqs2Global_NodeIndex[j];
                    double val =
                        GetNodeValue(k, nidx1) + damping * eqs_x[j + ish];
        ...
        /* JT: I don't know if this time control method is used anymore. But it
           relies on a single error produced from CalcIterationNodeError(), but
           this now depends on the type of error to use. Therefore, I simply
           provide the error of the first dof, and not depending on the error
           type. If this time step is still used, someone will need to find
           another way to calculate the error it uses.
        */
        Tim->repeat = true;
        if (converged)
        {
...
void CRFProcess::CalcFluxesForCoupling(void)
{
    ...
    for (i = 0; i < no_richards_problems; i++)
    {
        ...
        if (AverageZ_GW >= AverageZ_OLF)
        {
            ...
            // IndexBottomNode Achtung nur zum Testen Source für GW flow
            // durchgehend !!!!!! SetNodeValue(IndexBottomNode, idxFLUX, -flux);
            // //H_OLF  > H_GW -> + flux_GW
    ...
    for (i = 0; i < no_processes; i++)
    {
        ...
        if (m_pcs->getProcessType() == pcs_type)
        {
            ...
            if (testname.compare(comp_name) == 0)
            {
                ...
                //        compnumber " << m_pcs->pcs_component_number << "\n";
                pcs_no = i;
                return pcs_no;
            }
        }
...
void CRFProcess::CalcSecondaryVariablesTNEQ()
{
    ...
    const int idx_nodal_solid_density =
        this->GetNodeValueIndex("SOLID_DENSITY_N");
    ...
    for (i = 0; i < mesh_ele_vector_size; i++)
    {
        ...
        if (elem->GetMark())  // Marked for use
        {
            fem->UpdateSolidDensity(
                elem->GetIndex());  // HS, thermal storage reactions
            fem->ExtrapolateGauss_ReactRate_TNEQ_TES(
                *elem, this);  // HS added 19.02.2013
        }
    }
}
...
void CRFProcess::CalcSecondaryVariablesPSGLOBAL()
{
    ...
    for (i = 0; i < (long)m_msh->GetNodesNumber(false); i++)
    {
        ...
        if ((mmp->capillary_pressure_model == 4 ||
             mmp->capillary_pressure_model == 6) &&
...
node values of process pcs_name and for variable var_name; old and new timelevel
    01/2006   SB    Implementation
    02/2008   CB    generalization
**************************************************************************/
double PCSGetEleMeanNodeSecondary_2(long index, int pcsT,
        ...
        case 1313:  // Two_phase_Flow
            ...
            if (m_pcs)
            {
                idx = m_pcs->GetNodeValueIndex(
                    var_name);  // +timelevel; CB SATURATION1 in PS_GLOBAL is
                                // only for old time level
                cplpcs = m_pcs;
            }
            break;
                               ...
                               double* result)
{
    ...
    for (size_t i = 0; i < ele_vector_at_geo.size(); i++)
    {
        ...
        // 2D mesh in x-z-direction z-velocity is stored a y-position -> this is
        // corrected here
            ...
            (Edge_on_Geo == true))
        {
            vecConsideredEdges.push_back(
                m_edg->GetIndex());  // all edges that were already used are
                                     // stored
            ...
            if (pcs_type == FiniteElement::MULTI_PHASE_FLOW)  // BG, 04/2012
            {
                ...
                for (size_t j = 0; j < 3; j++)
                {
                    ...
                    // v_eidx_2[j]); Calculate Element velocity
                ...
                // it is a 2D mesh in x-z-direction z-velocity is stored a
                // y-position -> this is corrected here
                if ((dimension == 2) && (axis == 2))
                {
                    v_2[2] = v_2[1];
                    v_2[1] = 0;
    ...
    for (size_t i = 0; i < ele_vector_at_geo.size(); i++)
    {
        ...
        if (m_ele->GetDimension() == 2)
        {
                 ...
                 j++)
            {
                ...
                else
                {
                    ...
                    {
                        Point_on_Geo = false;
                        for (std::size_t l = 0; l < nod_vector_at_geo.size();
                             l++)
                        {
                            if (vec_nodes_edge[k]->GetIndex() ==
                                nod_vector_at_geo[l])
                    ...
                    (Edge_on_Geo == true))
                {
                    ...
                    // << "      J_adv_x: " << J_adv[0] << " J_adv_y: " <<
                    // J_adv[1] << " J_adv_z: " << J_adv[2] <<
                    // "\n";
                    ...
                    {
                        Disp_xx =
                            alpha_l * pow(v[0], 2) / norm_v +
                            alpha_t * pow(v[1], 2) / norm_v;  // ToDo:
                                                              // z-Dimension
                        Disp_yy =
                            alpha_l * pow(v[1], 2) / norm_v +
                            alpha_t * pow(v[0], 2) / norm_v;  // ToDo:
                                                              // z-Dimension
        ...
        if (m_ele->GetDimension() == 1)
        {
            ...
            if (Use_Element == true)
            {
                ...
                // cout << " element: " << m_ele->GetIndex() << " vx: " <<
                // vn_vec[0] << " vy: " << vn_vec[1] << " vz: "
                // << vn_vec[2] << " c1: " << ElementConcentration[0] << " c2: "
                // << ElementConcentration[1] << " c3: "
                // << ElementConcentration[2] << " c: " << C_ele << "\n";
...
void CRFProcess::AssembleParabolicEquationRHSVector(CNode* m_nod)
{
    ...
    for (size_t i = 0; i < 3; i++)
        if (v_eidx[i] < 0)
        {
            cout
                << v_eidx[i] << i
                << " Warning in CRFProcess::AssembleParabolicEquationRHSVector "
                   "- no PCS-VEL data"
                << "\n";
            return;
        }
                ...
                for (j = 0; j < (int)m_ele->GetEdgesNumber(); j++)
                {
                    ...
                    if (m_edg->GetMark())
                    {
                        ...
                        /*
                           ...
                           edge_mid_point[0]; aux_vector[1] = gravity_center[1]
                           - edge_mid_point[1]; aux_vector[2] =
                           gravity_center[2] - edge_mid_point[2]; check_sign =
...
void MMPCalcSecondaryVariablesNew(CRFProcess* m_pcs, bool NAPLdiss)
{
    ...
    for (i = 0; i < (long)m_pcs->m_msh->nod_vector.size(); i++)
    {
        ...
        // composition of NAPL phase CB phase 2
...
void CreateEQS_LinearSolver()
{
    ...
    for (i = 0; i < num_vector.size(); i++)
    {
        ...
        if (num->nls_method == 2)
        {
            ...
            // processes. Therefore, memo allocation is performed for it.
...
#ifdef USE_PETSC
            std::size_t id_in_eqs =
                m_msh->nod_vector[nindex]->GetEquationIndex();
#else
        ...
        for (size_t i = 0; i < size_st; i++)
        {
            ...
            os << id_in_eqs << "  "
               << convertPrimaryVariableToString(
                      st_node[i]->getProcessPrimaryVariable())
               << " " << std::setw(14) << pnt[0] << " " << std::setw(14)
               << pnt[1] << " " << std::setw(14) << pnt[2] << " "
...
void CRFProcess::UpdateTransientBC()
{
    ...
    if (precip)  // 08-07.06.2010.  WW
    {
        string ofile_name;
        ofile_name =
            precip->assignPrecipitationDirectlyOnTopSurfaceNodes(aktuelle_zeit);
        ...
        if (m_msh->GetCoordinateFlag() / 10 == 3)  // 19.08.2010. WW
        {
            ...
            for (int ii = 0; ii < 2; ii++)
            {
                ...
                if (pos > 0)
                    ofile_name.erase(ofile_name.begin() + pos,
                                     ofile_name.end());
            }
            ...
            for (std::size_t i = 0; i < m_msh->face_vector.size(); i++)
            {
                ...
                int row_min =
                    nrows - static_cast<int>((y_max - g_para[3]) / g_para[4]);
                int col_max = static_cast<int>((x_max - g_para[2]) / g_para[4]);
                int row_max =
                    nrows - static_cast<int>((y_min - g_para[3]) / g_para[4]);
    ...
    for (size_t i = 0; i < bc_transient_index.size(); i++)
    {
        ...
        MathLib::PiecewiseLinearInterpolation(
            ...
            nodes_as_interpol_points, interpol_res);
 ...
 **************************************************************************/
void CRFProcess::CalGPVelocitiesfromECLIPSE(string path, int timestep,
                                            int phase_index, string phase)
{
    (void)path;      // unused
    ...
    vec_string.push_back(
        "Element; X; Y; Z; v_Geosys_x; v_Geosys_y; v_Geosys_z; v_Eclipse_x; "
        "v_Eclipse_y; v_Eclipse_z");
    ...
    for (i = 0; i < (long)m_msh->ele_vector.size(); i++)
    {
        ...
        if (elem->GetMark())          // Marked for use
        {
            ...
            // cout << "Element: " << i << "\n";
            tempstring = fem->Cal_GP_Velocity_ECLIPSE(tempstring, true,
                                                      phase_index, phase);
    ...
    // string aus_file = path + "\\CheckVelocity_" + phase + "_" + tempstring +
    // ".csv"; ofstream aus; aus.open(aus_file.data(),ios::out); for (i = 0; i <
    // vec_string.size(); i++) {
    //	aus << vec_string[i] << "\n";
    //}
    // aus.close();
...
// Task: Calculates the isobaric vapour liquid equilibrium for the CO2-H2O-NaCl
// System Return: nothing Programming: 11/2010 DL, BG Modification:
//-------------------------------------------------------------------------
void CRFProcess::CO2_H2O_NaCl_VLE_isobaric(double T, double P,
                                           ...
                                           Phase_Properties& solid, int f)
{
    ...
    Molweight_NaCl =
        58.443;  // ToDo: provide constants once in the whole project
    ...
    if (f == 1 || f == 2)
    {
        ...
        if (tvCO2 == 0)
        {
            ...
            liquid.mass = liquid.H2O * Molweight_H2O +
                          liquid.CO2 * Molweight_CO2 +
                          liquid.NaCl * Molweight_NaCl;
        ...
        else
        {
            ...
            liquid.viscosity = -1.0;
            liquid.mass = liquid.CO2 * Molweight_CO2 +
                          liquid.H2O * Molweight_H2O +
                          liquid.NaCl * Molweight_NaCl;
            liquid.volume = liquid.mass / liquid.density;
        }
    }
...
void CRFProcess::CalculateFluidDensitiesAndViscositiesAtNodes(CRFProcess* m_pcs)
{
    ...
    double Molweight_CO2, Molweight_H2O, Molweight_NaCl;
    double Density_gas = 0, Density_liquid = 0, Density_pureCO2,
           Viscosity_liquid, Viscosity_gas;
    ...
    // for initial conditions variable index is the first index, for other
    // timesteps variable index is the second index
    ...
    for (long i = 0; i < (long)m_msh->nod_vector.size(); i++)
    {
        ...
        for (size_t j = 0; j < m_node->getConnectedElementIDs().size(); j++)
        {
            ...
            porosity =
                MediaProp->Porosity(m_ele->GetIndex(), m_pcs->m_num->ls_theta);
        ...
        //+1... new time level
        variable_index =
            m_pcs->GetNodeValueIndex("PRESSURE2") + TimeStepVariableIndex;
        // unit: bar!!! Assuming that the non wetting phase pressure is the
        // correct one
        ...
        //+1... new time level
        variable_index =
            m_pcs->GetNodeValueIndex("SATURATION1") + TimeStepVariableIndex;
        ...
        // Volume_eff = node_volume * (1 - saturation_gas_min -
        // saturation_liquid_min);		//[m³]
        //[m³]
        ...
        for (int j = 0; j < int(pcs_vector.size()); j++)
        {
            ...
            if ((pcs_MassTransport->nod_val_name_vector[0] == "C(4)") ||
                (pcs_MassTransport->nod_val_name_vector[0] ==
                 "CO2"))  // "CO2_w" CB
                MassTransportID[1] = j;
        ...
        // Read CO2 concentration in water and calculate moles of CO2 in liquid
        // phase
        ...
        //[mol/m³]
        c_CO2inLiquid =
            pcs_vector[indexProcess]->GetNodeValue(i, variable_index);

        // Read NaCl concentration in water and calculate moles of NaCl in
        // liquid phase
        ...
        if (saturation_liquid > 0)
        {
            ...
            // estimate density -> calculate moles of components -> calculate
            // real density -> compare with estimation -> loop until similar
            // values
            ...
            for (int j = 0; j < iter_max; j++)
            {
                ...
                // mass fraction of kg CO2 per kg liquid (mol/m³ * m³/kg * g/mol
                // / 1000) = kg/kg
                wCO2 = c_CO2inLiquid / temp_density * Molweight_CO2 / 1000;
                // mass fraction of kg NaCl per kg liquid (mol/m³ * m³/kg *
                // g/mol / 1000) = kg/kg
                wNaCl = c_NaClinLiquid / temp_density * Molweight_NaCl / 1000;
                wH2O =
                    1 - wCO2 - wNaCl;  // mass fraction of kg H2O per kg liquid
                // Molality = concentration in mol per kg of solution / mass
                // fraction of solvent = mol per kg solvent Molality of CO2 in
                // mol per kg pure water (mol/m³ * m³/kg) = mol/kg
                b_CO2inPureWater = c_CO2inLiquid / temp_density * 1 / wH2O;
                // Molality of NaCl in mol per kg pure water (mol/m³ * m³/kg) =
                // mol/kg
                b_NaClinPureWater = c_NaClinLiquid / temp_density * 1 / wH2O;
                // new estimate of density of brine with estimated concentration
                // of CO2 and NaCl in mol per kg pure water function provides
                // g/cm³ -> Density_liquid [kg/m³]
                Density_liquid = 1000 * density::CO2brine(temperature, pressure,
                                                          b_NaClinPureWater,
                                                          b_CO2inPureWater);
                // cout.precision(8);
                // cout << "Iteration: " << j << " estimated: " << temp_density
                // << " calculated: " << Density_liquid << " a: " << a  << " b:
            ...
            // c_H2OinLiquid = (Density_liquid - c_CO2inLiquid * Molweight_CO2 *
            // 1e-3 - c_NaClinLiquid * Molweight_NaCl
            ...
            // + 1;
            // // +1: new timelevel
            // pcs_vector[indexProcess]->SetNodeValue(i, variable_index,
            // c_H2OinLiquid);
        ...
        {
            // components in gas
            // Read H2O concentration in gas and calculate moles of H2O in gas
            // phase
            ...
            {
                temp_density = (a + b) / 2.0;
                // mass fraction of kg H2O per kg gas  (mol/m³ * m³/kg * g/mol /
                // 1000) = kg/kg
                ...
                // new estimate of density of brine with estimated concentration
                // of CO2 and NaCl in mol per kg pure water
                Density_pureCO2 =
                    1000 * VLE::density_CO2(temperature, pressure);
                Density_gas = Density_pureCO2;

                // Using Norberts EOS, uses mole fractions of components,
                // temperature and pressure [Pa]
                ...
                n_CO2 = m_CO2 * Molweight_CO2 /
                        1000;  // moles of CO2 [mol] = kg * g/mol / 1000 g/kg
                n_H2O = m_H2O * Molweight_H2O /
                        1000;  // moles of H2O [mol] = kg * g/mol / 1000 g/kg
                x_CO2 =
                    n_CO2 / (n_CO2 + n_H2O);  // mole fraction of CO2 [mol/mol]
                // WW x_H2O = n_H2O / (n_CO2 + n_H2O);      // mole fraction of
                // H2O [mol/mol] ToDo: reorganisation of the code [kg/m³]
                Density_gas = DuansMixingRule(temperature, pressure * 1e5,
                                              x_CO2, 0, 1, 0);
                // cout.precision(8);
                // cout << "Iteration: " << j << " estimated: " << temp_density
                // << " calculated: " << Density_gas << " a: " << a  << " b: "
                // << b << "\n";
        ...
        // cout << "   node: " << i << " Dichte_Wasser: " << Density_liquid << "
...
// Task: Calculations Phasetransition for CO2 between gas and liquid phase,
// calls a DLL Return: nothing Programming: 11/2010 BG, NB Modification:
//-------------------------------------------------------------------------
void CRFProcess::Phase_Transition_CO2(CRFProcess* m_pcs, int Step)
{
    ...
    for (long i = 0; i < (long)m_msh->nod_vector.size(); i++)
    {
        ...
        // unit: bar!!! Assuming that the non wetting phase pressure is the
        // correct one
        gas.pressure = liquid.pressure = solid.pressure =
            m_pcs->GetNodeValue(i, variable_index) / 1e5;
        //+1... new time level
        ...
        saturation_liquid_effective =
            (saturation_liquid - saturation_liquid_min) /
        ...
        //[m³]
        Volume_eff =
            node_volume * (1 - saturation_gas_min - saturation_liquid_min);
        //[m³]
        liquid.volume = Volume_eff * saturation_liquid_effective;
        //[m³]
        gas.volume = Volume_eff * saturation_gas_effective;

        // cout << "node: " << i << " p_cap: " << p_cap << " P2: " <<
        // gas.pressure << " sat_liquid: " << saturation_liquid << " sat_gas: "
        // << saturation_gas << " V_liquid: " << liquid.volume << " V_gas: " <<
        // gas.volume << "\n";
        ...
        // get concentration of phase components and calculate moles of
        // components per phase Read CO2 concentration in water and calculate
        // moles of CO2 in liquid phase
        indexProcess = MassTransportID[1];
        solid.CO2 = 0;
        // +1: new timelevel
        variable_index =
        ...
        // variable_index = m_pcs->GetNodeValueIndex("DENSITY1_old"); //+1...
        // new time level if (m_pcs->Tim->step_current == 1)
        //	Density_liquid_old = Density_liquid;
        // else
        //	Density_liquid_old = m_pcs->GetNodeValue(i, variable_index);
        //// [kg/m³
        ...
        // variable_index =
        // pcs_vector[indexProcess]->GetNodeValueIndex(pcs_vector[indexProcess]->pcs_primary_function_name[0])
        // + TimeStepVariableIndex; // +1: new timelevel c_CO2inLiquid_old =
        // pcs_vector[indexProcess]->GetNodeValue(i, variable_index);
        // //[mol/m³] if (i == 230)
        //	cout << i << "\n";
        // liquid.H2O = (Density_liquid_old * liquid.volume - c_CO2inLiquid_old
        // * liquid.volume  * Molweight_CO2 * 1e-3
        ...
        if (Step == 1)
        {
            // phase transition after reactions -> use old liquid.H2O because no
            // phase movement
            ...
            // calculate new liquid.H2O after multiphase flow calculation based
            // on new pressure, new density because of this new pressure, new
            // phase saturations
            //[mol]
            liquid.H2O = (Density_liquid * liquid.volume -
                          liquid.CO2 * Molweight_CO2 * 1e-3 -
                          liquid.NaCl * Molweight_NaCl * 1e-3) /
                         (Molweight_H2O * 1e-3);

        // if (m_pcs->Tim->step_current == 1)
        // liquid.H2O = (Density_liquid * liquid.volume - liquid.CO2 *
        // Molweight_CO2 * 1e-3 - liquid.NaCl * Molweight_NaCl * 1e-3 ) /
        // (Molweight_H2O * 1e-3);	//[mol] else { indexProcess =
        // MassTransportID[0]; variable_index =
        // pcs_vector[indexProcess]->GetNodeValueIndex(pcs_vector[indexProcess]->pcs_primary_function_name[0])
        // + 1; // +1: new timelevel c_H2OinLiquid =
        // pcs_vector[indexProcess]->GetNodeValue(i, variable_index);
        ...
        if (Step == 1)
        {
            ...
            // calculate new liquid.H2O after multiphase flow calculation based
            // on new pressure, new density because of this new pressure, new
            // phase saturations [mol] = kg / (kg / mol)
            gas.CO2 =
                (Density_gas * gas.volume - gas.H2O * Molweight_H2O * 1e-3) /
                (Molweight_CO2 * 1e-3);
        ...
        {
            cout << "The volume is not equal before and after the calculation "
                    "of CO2 phase transition! "
                 << "\n";
            cout << "Before: " << Volume_eff
                 << " After: " << gas.volume + liquid.volume << "\n";
            // system("Pause");
            exit(0);
        }
        ...
        // saturation_liquid = max(liquid.volume / Volume_eff,
...
for(i = 0; i < nelements; i++) {
  ...
  if (elem->GetMark()){
    ...
    for(gp=0;gp<n_gauss;gp++){
      ...
      //for(i_dim=0;i_dim < ele_dim;i_dim++) cout << " i_dim " << i_dim << ",
vel:   " << vel[i_dim] << "\n";
...
<< "\n";
      ...
      for(i_dim=0; i_dim < ele_dim; i_dim++){
        for(int l=0; l<nnodes; l++){
            gradConc[i_dim] += node_concs[l]*
m_fem->Getdshapefct(i_dim*nnodes+l);
            //if(i< 5) cout << "i_dim, l, gradConc, node_concs, shapefunction :
" << i_dim << ",  " << l ;
            //if(i< 5) cout << " ,  " << gradConc[i_dim] << ", " <<
node_concs[l] << " ,  " << m_fem->Getdshapefct(i_dim*m_fem->Getnnodes()+l) <<
"\n";
        }
      }
      ...
      for(i_dim=0; i_dim < ele_dim; i_dim++){
        ...
        //if(i<5) cout << el_field[i_dim] << "  " << sum_gradC[i_dim] << "  " <<
sum_vC[i_dim] << "  "  << sum_C <<
"\n";
