    ...
    defined(USE_MPI)  //|| defined(other parallel libs)//03.3012. WW
        ...
        if (line_string.find("#OUTPUT") != string::npos)
        {
            ...
            if (output_version)  //// 02.2011. WW
            {
                ...
                out->setFileBaseName(out->getFileBaseName() + "(V" +
                                     VersionStr + ")");
            }
...
void OUTData(double time_current, int time_step_number, bool force_output)
{
    ...
    for (size_t i = 0; i < out_vector.size(); i++)
    {
        ...
        if (m_out->doesTheVariableExist("TEMPERATURE1"))
        {
            ...
            if (T_process &&
                T_process->getTemperatureUnit() == FiniteElement::CELSIUS)
            {
                const int T_idx =
                    T_process->GetNodeValueIndex("TEMPERATURE1") + 1;
                for (std::size_t i = 0;
                     i < T_process->m_msh->GetNodesNumber(false);
                     i++)
                {
                    T_process->SetNodeValue(
                        i,
                        T_idx,
                        T_process->GetNodeValue(i, T_idx) -
    ...
    for (size_t i = 0; i < out_vector.size(); i++)
    {
        ...
        if (!m_pcs)
        {
            // In case an output is defined for an undefined procsse, i.e. not
            // given in pcs file.
            ...
            switch (m_out->getGeoType())
            {
                case GEOLIB::GEODOMAIN:  // domain data
                    ...
                    if (OutputBySteps)
                    {
                        ...
                        if (m_out->dat_type_name.compare("BINARY") ==
                            0)  // 08.2012. WW
                        {
        ...
        else if (m_out->dat_type_name.find("PVD") != string::npos)
        {
            ...
            switch (m_out->getGeoType())
            {
                case GEOLIB::GEODOMAIN:  // domain data
                {
                    ...
                    std::string pvd_vtk_file_path = pathJoin(
                        vtk->pvd_vtk_file_path_base, pvd_vtk_file_name);

                    // Output
                    if (OutputBySteps)
                    {
                        vtk->WriteXMLUnstructuredGrid(
                            pvd_vtk_file_path, m_out, time_step_number);
                        VTK_Info dat;
            ...
            m_out->NODWriteTotalFlux(time_current,
                                     time_step_number);  // 6/2012 JOD, MW
    ...
    if (T_process && T_process->getTemperatureUnit() == FiniteElement::CELSIUS)
    {
        ...
        for (std::size_t i = 0; i < T_process->m_msh->GetNodesNumber(false);
             i++)
        {
            T_process->SetNodeValue(i,
                                    T_idx,
                                    T_process->GetNodeValue(i, T_idx) +
                                        PhysicalConstant::CelsiusZeroInKelvin);
        }
...
/***************************************************************************************
 ...
 input file This means up to now, that data for missing processes is not written
   05/09	SB
 *****************************************************************************************/
void OUTCheck()
{
