...
public:
    ...
    const std::vector<GEOLIB::Station*>& getClimateStations() const
    {
        return this->_weather_stations;
    };  // NB
    // 03.2010. WW
    std::string assignPrecipitationDirectlyOnTopSurfaceNodes(
        double current_time);
    ...
    Constrained const& getConstrainedST(std::size_t constrainedID) const
    {
        return _constrainedST[constrainedID];
    }
    std::size_t getNumberOfConstrainedSTs() const
    {
        return _constrainedST.size();
    }
    ...
    void pushBackConstrainedSTNode(std::size_t constrainedID,
                                   bool constrainedStatus)
    {
        _constrainedST[constrainedID]._constrainedNodes.push_back(
            constrainedStatus);
    }
    ...
    void setConstrainedSTNode(std::size_t constrainedID, bool node_status,
                              int position)
    {
    ...
    int getConstrainedSTNodesIndex(int position) const
    {
        return _constrainedSTNodesIndices[position];
    }
    ...
    bool isPressureBoundaryCondition() const
    {
        return pressureBoundaryCondition;
    }
    void SetPressureBoundaryConditionFlag(bool pBC)
    {
        pressureBoundaryCondition = pBC;
    }
    ...
    double air_breaking_factor, air_breaking_capillaryPressure,
    ...
    std::vector<double> get_node_value_vectorArea()
    {
        return node_value_vectorArea;
    }  // TN
    std::vector<long> st_node_ids;

private:  // TF, KR
    void ReadDistributionType(std::ifstream* st_file);
    void ReadGeoType(std::ifstream* st_file, const GEOLIB::GEOObjects& geo_obj,
                     const std::string& unique_name);
    ...
    // including climate data into source terms
    MathLib::InverseDistanceInterpolation<GEOLIB::PointWithID*,
                                          GEOLIB::Station*>* _distances;  // NB
...
public:
     ...
     * flow)</td></tr> <tr><td>RIVER_FLOW</td> <td>H process (incompressible
     * flow)</td></tr> <tr><td>RICHARDS_FLOW</td> <td>H process (incompressible
     * flow)</td></tr> <tr><td>OVERLAND_FLOW</td> <td>process (incompressible
     * flow)</td></tr> <tr><td>GAS_FLOW</td> <td>H process (compressible
     * flow)</td></tr> <tr><td>TWO_PHASE_FLOW</td> <td>H2 process
     * (incompressible/compressible flow)</td></tr>
     * <tr><td>COMPONENTAL_FLOW</td> <td>H2 process (incompressible/compressible
     * flow)</td></tr> <tr><td>HEAT_TRANSPORT</td> <td>T process
     * (single/multi-phase flow)</td></tr> <tr><td>DEFORMATION</td> <td>M
     * process (single/multi-phase flow)</td></tr> <tr><td>MASS_TRANSPORT</td>
     * <td>C process (single/multi-phase flow)</td></tr>
     * </table>
     */
    std::string pcs_name;
...
private:
     ...
     */
    void SetPolylineNodeValueVector(
        CSourceTerm* st,
    ...
    void AreaAssembly(const CSourceTerm* const st,
                      const std::vector<long>& ply_nod_vector_cond,
                      std::vector<double>& ply_nod_val_vector) const;
    void DistributeVolumeFlux(CSourceTerm* st,
                              std::vector<long>& ply_nod_vector,  // 5.3.07
                              std::vector<double>& ply_nod_val_vector);

#if defined(USE_PETSC)  // || defined (other parallel linear solver lib). //WW.
                        // 06.2016
    void removeGhostSourceTerms(std::vector<long>& node_ids,
...
#endif
// JOD
extern double CalcCouplingValue(double factor, double h_this, double h_cond,
