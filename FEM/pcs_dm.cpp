    ...
    for (std::size_t i = 0; i < m_msh->ele_vector.size(); i++)
    {
        ...
        for (int j = 0; j < nnodes; j++)
        {
            ...
            if (h_pcs_type == 14)  // richards flow
            {
                if (SMat->bishop_model == 1)
                    pw += tmp_h_pcs->GetNodeValue(j, idx_p1) *
                          SMat->bishop_model_value;
                else if (SMat->bishop_model == 2)
                    pw += tmp_h_pcs->GetNodeValue(j, idx_p1) *
                          pow(tmp_h_pcs->GetNodeValue(j, idx_s),
                              SMat->bishop_model_value);
                else if (SMat->bishop_model == 3)
                {
                    tmp_h_pcs->GetNodeValue(j, idx_s) >=
                            SMat->bishop_model_value
                        ? pw += tmp_h_pcs->GetNodeValue(j, idx_p1)
                        : pw += 0.0;
                }
            ...
            if (h_pcs_type == 1212 ||
                h_pcs_type == 42)  // multi phase flow  pg-sw*pc
            {
                if (SMat->bishop_model == 1)
        ...
        if (elem->GetMark())  // Marked for use, it is necessary here
        {
            ...
            for (int gp = 0; gp < NGS; gp++)
            {
                for (int j = 0; j < 3; j++)
                {
                    if (SMat->biot_const < 0 &&
                        pw < 0)  // if biot is negative value, negative pw is
                                 // not considered
                        (*eleV_DM->Stress0)(j, gp) = (*eleV_DM->Stress0)(j, gp);
                    else
                        (*eleV_DM->Stress0)(j, gp) =
                            (*eleV_DM->Stress0)(j, gp) - SMat->biot_const * pw;
                }
                // for postExcavation Stress0 is already updated with Stress, as
                // well as UpdateIniStateValue()
            }
        }
    }  // end loop elements
}
...
//
    ...
    /*
       ...
       //  Activate this segement if neccessary as well as Auto increment
       loading  2
       //---------------------------------------------------------------------
    ...
    for (int l = 1; l <= number_of_load_steps; l++)
    {
        ...
        //     Prepared for the special usage. i.e, for test purpose.
        //     Agitate this segement if neccessary as well as Auto increment
        //     loading  1
        //---------------------------------------------------------------------
        /*
           if(elasticity!=1&&number_of_load_steps>1)
           {
            // Predictor the size of the load increment, only perform at the
           first calling minLoadRatio = (double)l/(double)number_of_load_steps;
           ...
           //TEST     else if(fluid)   LoadFactor =
           (double)l/(double)number_of_load_steps;
         */
            ...
            if (!elasticity)
            {
                ...
                if (ite_steps == 1)
                {
                    ...
                    pcs_relative_error[0] =
                        pcs_absolute_error[0] / Tolerance_global_Newton;
                    // To be improved by overloading
                    // CRFProcess::CalcIterationNODError
                    cpl_max_relative_error =
                        pcs_absolute_error[0];  // pcs_relative_error[0];
...
//
    ...
    j++) if(l == node->getConnectedElementIDs()[j])
                    {
                        done = true;
                        break;
                    }
...
void CRFProcessDeformation::InitGauss(void)
{
    ...
    // Initial stresses are given in .ic file, reload file is therefore
    // disabled,
    if (ccounter > 0)
    {
            ...
            if (u_type == 0)
            {
                ...
                for (j = 0; j < number_of_nodes; j++)
                {
                    ...
                    else
                    {
                        ...
                        eqs_r = pcs_number_of_primary_nvals * size_l +
                                problem_dimension_dm * (ja - size_l) + i;
                    }
                    ...
                    SetNodeValue(
                        j,
                        ColIndex,
                        GetNodeValue(j, ColIndex) + eqs_x[eqs_row] * damp);
                }
                shift += number_of_nodes;
            }
            else
...
#if defined(USE_PETSC)  // || defined (other parallel solver lib). 06.2013 WW
                const long eqs_row =
                    problem_dimension_dm * m_msh->Eqs2Global_NodeIndex[j] + i;
#else
                const long eqs_row = j + shift;
#endif
                SetNodeValue(j,
        ...
        else
        {
            for (j = 0; j < number_of_nodes; j++)
                SetNodeValue(
                    j,
                    ColIndex + 1,
                    GetNodeValue(j, ColIndex + 1) + GetNodeValue(j, ColIndex));
        }
    }
        ...
        for (i = problem_dimension_dm; i < pcs_number_of_primary_nvals; i++)
        {
            ...
            for (j = 0; j < number_of_nodes; j++)
            {
                SetNodeValue(
                    j,
                    ColIndex,
                    GetNodeValue(j, ColIndex) + eqs_x[j + shift] * damp);
            }
            shift += number_of_nodes;
        }
    }
    ...
    for (i = 0; i < (long)m_msh->ele_vector.size(); i++)
    {
        ...
        if (elem->GetMark())  // Marked for use
        {
            ...
            for (int i = 0; i < fem_dm->nnodes; i++)
            {
                ...
                double stress[6];
                stress[0] = fem_dm->pcs->GetNodeValue(
                    node_index, Idx_Stress[0]);  // sigma_xx
                stress[1] = fem_dm->pcs->GetNodeValue(
                    node_index, Idx_Stress[1]);  // sigma_yy
                stress[2] = fem_dm->pcs->GetNodeValue(
                    node_index, Idx_Stress[2]);  // sigma_zz
                stress[3] = fem_dm->pcs->GetNodeValue(
                    node_index, Idx_Stress[3]);  // sigma_xy
                if (problem_dimension_dm == 3)
                {
                    stress[4] = fem_dm->pcs->GetNodeValue(
                        node_index, Idx_Stress[4]);  // sigma_xz
                    stress[5] = fem_dm->pcs->GetNodeValue(
                        node_index, Idx_Stress[5]);  // sigma_yz
                }
                else
                {
                    stress[4] = 0;  // sigma_xz
                ...
                for (size_t i = 0; i < 3; i++)
                    fem_dm->pcs->SetNodeValue(
                        node_index, stressPrincipleIndices[i], prin_str[i]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[0], prin_dir[0]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[1], prin_dir[3]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[2], prin_dir[6]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[3], prin_dir[1]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[4], prin_dir[4]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[5], prin_dir[7]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[6], prin_dir[2]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[7], prin_dir[5]);
                fem_dm->pcs->SetNodeValue(
                    node_index, PrinStressDirectionIndices[8], prin_dir[8]);
            }
        }
    }
}
...
void CRFProcessDeformation::GlobalAssembly()
{
    ...
    // WW
    {
        ...
        //----------------------------------------------------------------------
        //
        // {        		MXDumpGLS("rf_pcs1.txt",1,eqs->b,eqs->x); //
        // abort();}
        ...
        /// If not JFNK or if JFNK but the Newton step is greater than
        /// one. 11.11.2010. WW
