      ...
      msize(0)
{
    ...
    if (!validMatID)
    {
        std::cout << "Error: not enough MMP data. please check MMP and "
                     "material IDs in a mesh."
                  << std::endl;
    ...
    {
        pcs_fluid_momentum->_idxVx =
            pcs_fluid_momentum->GetNodeValueIndex("VELOCITY1_X", true);
        pcs_fluid_momentum->_idxVy =
            pcs_fluid_momentum->GetNodeValueIndex("VELOCITY1_Y", true);
        pcs_fluid_momentum->_idxVz =
    ...
    if (REACTINT_vec.size() == 0)
    {
        for (size_t i = 0; i < mmp_vector.size(); i++)
        {
            ...
            {
                std::cout
                    << " Error in Model setup: Porosity model 13 is used, "
                    << "\n";
                std::cout
                    << " but no reaction interface is defined! Exiting now..."
                    << "\n";
                exit(0);
            }
        }
    ...
    {
        // set the id variable flow_pcs_type for Saturation and velocity
        // calculation in mass transport element matrices
        SetFlowProcessType();
        //----------------------------------------------------------------------
        ...
        if (REACTINT_vec.size() > 0)
        {
            if (REACTINT_vec[0]->unitconversion)
            {
                ...
                flow_pcs = PCSGetFlow();
                if (flow_pcs->type ==
                    1212)  // in case of mutlltiphase flow, sat water must be
                           // calculated here, required by pgc interface
...
// SB4501        rc->ExecuteReactions();
    ...
    if (REACT_CAP_vec.size() > 0)
    {
        ...
        REACT_CAP_vec[0]->ExecuteReactionsChemApp(
            0, -1);  // DL/SB 11/2008 //DL 2011.11.24 comment for AGU
...
// If shared memory, skip the following line
    ...
    start_time =
        1.0e+25;  // 1.e+8;  kg44 I need a really big time, as I have starting
                  // times bigger than 3.e+13 (1 Million years!!!)
    end_time = 0.;
    max_time_steps = 0;
    bool time_ctr = false;
    ...
    for (size_t i = 0; i < time_vector.size(); i++)
    {
        ...
        // check maximum number of coupling iterations against maximum time step
        // increase
        if (m_tim->time_control_type == TimeControlType::SELF_ADAPTIVE &&
            ...
            std::cout << "Warning: Maximum number of coupling iterations is "
                         "smaller than maximum time step increase!!!"
                      << std::endl;
    }
    if (max_time_steps == 0)
    ...
    for (size_t i = 0; i < no_processes; i++)
    {
        ...
        if (m_pcs->getProcessType() ==
            FiniteElement::FLUID_MOMENTUM)  // 09.2012 WW
        {
            fluid_mom_pcs = true;
            break;
        }
...
void Problem::SetActiveProcesses()
{
    ...
    for (size_t i = 0; i < max_processes; i++)
        if (total_processes[i])
        {
            ...
            // necessary for multiple component transport situations. First try
            // for a variable, because this is more general for mass transport
    ...
    {  // WW
            ...
            pcs->getProcessType() == FiniteElement::DEFORMATION_H2)
        {
            CRFProcessDeformation* dm_pcs =
                dynamic_cast<CRFProcessDeformation*>(pcs);
            dm_pcs->InitGauss();
        }
    }
...
// Output zero time initial values
    ...
    while (end_time > current_time)
    {
        ...
        for (i = 0; i < (int)active_process_index.size(); i++)
        {
            ...
            dt_rec =
                MMin(dt_rec,
                     m_tim->recommended_time_step);  // to know if we have a
                                                     // critical time alteration
        }
...
//
        ...
        MPI_Bcast(&dt,
                  ...
                  MPI_DOUBLE,
                  0,
                  comm_DDC);  // all processes use the same time stepping
                              // (JT->WW. Must they always?)
#endif
        //
        ...
        else
        {
            ...
            for (i = 0; i < (int)total_processes.size(); i++)
            {
                if (!active_processes[i] && total_processes[i] &&
                    total_processes[i]->tim_type == TimType::STEADY)
                {
...
bool Problem::CouplingLoop()
{
    ...
    // mean we don't want output for the others. if(acounter == num_processes)
    print_result = true;
    //
    bool accept = true;
    max_outer_error = 0.0;
...
#endif
                MPI_Allreduce(&has_constrained_bc_i,
                              &n_has_constrained_bc,
                              1,
                              MPI_INT,
                              MPI_SUM,
                              comm);
                if (n_has_constrained_bc > 0)
/* Commented out the following codes because MPI_C_BOOL is supported from the
MPI 2.2 bool has_constrained_bc_i = a_pcs->hasConstrainedBC(); bool
has_constrained_bc = false; MPI_Allreduce(&has_constrained_bc_i,
&has_constrained_bc, 1, MPI_C_BOOL, MPI_LOR, MPI_COMM_WORLD);
...
*/
        ...
        // tolerance.	//MW outer_index + 2 is hard to follow - is it faster to
        // write it like this?
...
{
    if (!last_dt_accepted ||
        force_post_node_copy)  // if last time step not accepted or values were
                               // already copied.
        return;
    //
    /*For mass transport this routine is only called once (for the overall
      transport process) and so we need to copy for all transport components*/
    if (m_pcs->getProcessType() == FiniteElement::MASS_TRANSPORT)
    {
...
void Problem::PostCouplingLoop()
{
    ...
    if ((KinReactData_vector.size() > 0) || (REACT_vec.size() > 0) || capvec ||
        prqvec)
    {
        ...
        if (KinReactData_vector.size() > 0)
        {
            if (KinReactData_vector[0]->NumberMineralkinetics > 0)
                KinReactData_vector[0]
                    ->PostprocessMinKin();  // Set Mineral surface areas for
                                            // next time step based on deltaC
        }
        m_pcs = PCSGetFlow();
...
inline double Problem::LiquidFlow()
{
    ...
    if (m_pcs->simulator.compare("ECLIPSE") ==
        0)  // use ECLIPSE to calculate one phase liquid flow, BG
    {
        if (m_pcs->EclipseData ==
            NULL)  // SBG if this is the first call, make a new instance
            m_pcs->EclipseData = new CECLIPSEData();
        // call ECLIPSE interface
        success =
            m_pcs->EclipseData->RunEclipse(m_pcs->Tim->step_current, m_pcs);
        if (success == 0)
...
inline double Problem::MultiPhaseFlow()
{
    ...
    if (m_pcs->simulator.compare("ECLIPSE") ==
        0)  // use ECLIPSE to calculate multi-phase flow, BG
    {
        if (m_pcs->EclipseData ==
            NULL)  // SBG if this is the first call, make a new instance
            m_pcs->EclipseData = new CECLIPSEData();
        ...
        m_pcs->EclipseData->dissolved_co2_pcs_name_ECL =
            m_pcs->dissolved_co2_pcs_name;  // hand over process name for
                                            // storing dissolved CO2
        m_pcs->EclipseData->dissolved_co2_ingas_pcs_name_ECL =
            m_pcs->dissolved_co2_ingas_pcs_name;
        ...
        ((m_pcs->simulator.compare("GEOSYS") == 0)))
    {
        ...
        if (m_pcs->Tim->step_current == 1)
        {
            ...
            {
                std::cout
                    << "If the Phase_Transition_Model is used the density "
                       "model and the viscosity model should be 18!"
                    << "\n";
...
void Problem::TestOutputDuMux(CRFProcess* m_pcs)
{
    ...
    if (m_pcs->Tim->step_current == 1)
        ...
        vec_string.push_back(
            "Time, massCO2_gas, massCO2_water, massCO2, porosity");
    else
    {
    ...
    indexConcentration_CO2 =
        pcs_vector[m_pcs->DuMuxData->ProcessIndex_CO2inLiquid]
            ->GetNodeValueIndex(
                pcs_vector[m_pcs->DuMuxData->ProcessIndex_CO2inLiquid]
    ...
    for (long i = 0; i < (long)m_msh->nod_vector.size(); i++)
    {
        ...
        concentration_CO2_water =
            pcs_vector[m_pcs->DuMuxData->ProcessIndex_CO2inLiquid]
                ->GetNodeValue(i, indexConcentration_CO2);
    ...
    for (long i = 0; i < (long)m_pcs->DuMuxData->NodeData.size(); i++)
    {
        ...
        saturation_water =
            m_pcs->DuMuxData->NodeData[i]->getPhaseSaturation()[0];
        saturation_CO2 = 1 - saturation_water;

        // concentration_CO2_water =
        // pcs_vector[m_pcs->DuMuxData->ProcessIndex_CO2inLiquid]->GetNodeValue(i,
        // indexConcentration_CO2);
        concentration_CO2_water =
            m_pcs->DuMuxData->NodeData[i]->getCO2InLiquid() *
...
void Problem::TestOutputEclipse(CRFProcess* m_pcs)
{
    ...
    // +1: new timelevel
    indexConcentration_CO2 =
        pcs_vector[m_pcs->EclipseData->ProcessIndex_CO2inLiquid]
            ->GetNodeValueIndex(
                pcs_vector[m_pcs->EclipseData->ProcessIndex_CO2inLiquid]
                    ->pcs_primary_function_name[0]) +
        1;
    ...
    for (long i = 0; i < long(m_pcs->EclipseData->eclgrid.size()); i++)
    {
        ...
        if (m_pcs->EclipseData->E100 == true)
            density_CO2 =
                m_pcs->EclipseData
                    ->Data[i][m_pcs->EclipseData->GetVariableIndex("GAS_DEN")];
        else
            density_CO2 =
                m_pcs->EclipseData
                    ->Data[i][m_pcs->EclipseData->GetVariableIndex("DENG")];
        ...
        if (m_pcs->EclipseData->E100 == true)
            ...
            saturation_water =
                m_pcs->EclipseData
                    ->Data[i][m_pcs->EclipseData->GetVariableIndex("SWAT")];
        saturation_CO2 =
            m_pcs->EclipseData
                ->Data[i][m_pcs->EclipseData->GetVariableIndex("SGAS")];
        ...
        for (int j = 0; j < int(ele_nodes.Size()); j++)
            concentration_CO2_water =
                concentration_CO2_water +
                pcs_vector[m_pcs->EclipseData->ProcessIndex_CO2inLiquid]
                        ->GetNodeValue(ele_nodes[j]->GetIndex(),
                                       indexConcentration_CO2) /
                    ele_nodes.Size();
...
files Return: nothing Programming: 05/2011 BG
-------------------------------------------------------------------------*/
...
// if (Windows_System == true)
    ...
    for (long i = 0; i < (long)m_msh->nod_vector.size(); i++)
    {
        ...
        // << " Concentration: " << ComponentConcentration << "\n";
    }
    ...
    if (REACTINT_vec.size() > 0)
        if (REACTINT_vec[0]->heatpump_2DhTO2Dv)
            REACTINT_vec[0]->Heatpump_2DhTO2Dv_Mapping(
                false);  // Restore the old solution
...
inline double Problem::MassTrasport()
{
    ...
    for (int i = 0; i < (int)transport_processes.size(); i++)
    {
        ...
        // MW reduce CONCENTRATION1 to non-negative values above water table for
        // stability for Sugio approach with RICHARDS
    ...
    // 0) REACT *rc = NULL; //OK rc = REACT_vec[0]; //OK
    ...
    // if( (KinReactData_vector.size() > 0) || (REACT_vec.size()>0) ||  capvec
    // || (REACT_PRQ_vec.size()>0) )  //CB merge CAP 0311
    if (REACTINT_vec.size() > 0)
    ...
    {  // WW moved the following lines into this curly braces. 12.12.2008
        ...
        // Calculate Chemical reactions, after convergence of flow and transport
        // Move inside iteration loop if couplingwith transport is implemented
        // SB:todo First calculate kinetic reactions
                                                    ...
                                                    int loop_process_number)
{
    ...
    if (aktueller_zeitschritt == 1)
    {
        ...
        m_pcs_local->fem->setShapeFunctionPool(
            m_pcs_global->fem->getShapeFunctionPool(0),
...
//------------
    ...
    // for(i=0;i<2;i++)
    {
        ...
        if (neighb_grid)
        {
            ...
            {
                m_st =
                    m_pcs_local->st_node[(int)m_pcs_local->st_node.size() - 1];
                m_nod_val =
                    m_pcs_local
                        ->st_node_value[(int)m_pcs_local->st_node_value.size() -
                                        1];
            }
...
void Problem::createShapeFunctionPool()
{
    ...
    if (pcs_1c_fem)
    {
        ...
        CRFProcessDeformation* dm_pcs =
            dynamic_cast<CRFProcessDeformation*>(pcs_1c_fem);
    ...
    for (std::size_t i = 0; i < fem_msh_vector.size(); i++)
    {
        ...
        if (mesh->getNumberOfLines() > 0)
            elem_types[static_cast<int>(MshElemType::LINE) - 1] =
                MshElemType::LINE;
        ...
        {
            elem_types[static_cast<int>(MshElemType::QUAD) - 1] =
                MshElemType::QUAD;
            elem_types[static_cast<int>(MshElemType::LINE) - 1] =
                MshElemType::LINE;
        }
        ...
        {
            elem_types[static_cast<int>(MshElemType::HEXAHEDRON) - 1] =
                MshElemType::HEXAHEDRON;
            elem_types[static_cast<int>(MshElemType::QUAD8) - 1] =
                MshElemType::QUAD8;
            elem_types[static_cast<int>(MshElemType::LINE) - 1] =
                MshElemType::LINE;
        }
        ...
        {
            elem_types[static_cast<int>(MshElemType::TETRAHEDRON) - 1] =
                MshElemType::TETRAHEDRON;
            elem_types[static_cast<int>(MshElemType::TRIANGLE) - 1] =
                MshElemType::TRIANGLE;
            elem_types[static_cast<int>(MshElemType::LINE) - 1] =
                MshElemType::LINE;
        ...
        {
            elem_types[static_cast<int>(MshElemType::PRISM) - 1] =
                MshElemType::PRISM;
            elem_types[static_cast<int>(MshElemType::QUAD8) - 1] =
                MshElemType::QUAD8;
            elem_types[static_cast<int>(MshElemType::TRIANGLE) - 1] =
        ...
        {
            elem_types[static_cast<int>(MshElemType::PYRAMID) - 1] =
                MshElemType::PYRAMID;
            elem_types[static_cast<int>(MshElemType::QUAD8) - 1] =
                MshElemType::QUAD8;
            elem_types[static_cast<int>(MshElemType::TRIANGLE) - 1] =
    ...
    if (lin_fem_assembler)
        _linear_shapefunction_pool = new FiniteElement::ShapeFunctionPool(
            elem_types, *lin_fem_assembler, num_gauss_sample_pnts);
    if (fem_assembler)
    {
        _quadr_shapefunction_pool = new FiniteElement::ShapeFunctionPool(
            elem_types, *fem_assembler, num_gauss_sample_pnts);
        if (!_linear_shapefunction_pool)
        {
    ...
    for (std::size_t i = 0; i < pcs_vector.size(); i++)
    {
        ...
        if (pcs->getProcessType() == FiniteElement::DEFORMATION)
        {
            ...
            fem_assem_h->setShapeFunctionPool(_linear_shapefunction_pool,
                 ...
                 pcs->getProcessType() == FiniteElement::DEFORMATION_H2)
        {
            ...
            fem_assem->setShapeFunctionPool(_linear_shapefunction_pool,
                                            _quadr_shapefunction_pool);
        }
        else
        {
...
// iteration, we need to get the timing information.
