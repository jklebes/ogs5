...
namespace
{
static inline double time_interpolate(double const* const a,
                                      double const* const b, double theta,
...
// 27.2.2007. GravityMatrix = NULL;
    ...
    switch (pcs->getProcessType())
    {
        ...
        case FLUID_MOMENTUM:
        case FLUX:
            PcsType = EPT_RICHARDS_FLOW;  // R should include L if the eqn of R
                                          // is written right.
            break;
        ...
        case MULTI_COMPONENTIAL_FLOW:
            for (int in = 0; in < pcs->pcs_number_of_primary_nvals; in++)
            {
                ...
                idxMCF[in] =
                    pcs->GetNodeValueIndex(pcs->pcs_primary_function_name[in]);
...
void CFiniteElementStd::SetMemory()
{
    ...
    if (PcsType == EPT_MULTIPHASE_FLOW ||
        PcsType == EPT_PSGLOBAL)  // 4.3.2009 PCH
        Size *= 2;
    ...
    if (pcs->Memory_Type == 0)
    {
        if (PcsType == EPT_MULTIPHASE_FLOW || PcsType == EPT_PSGLOBAL ||
            ...
            Mass->LimitSize(nnodes,
                            nnodes);  // Mass->LimitSize(nnodes); // unsymmetric
                                      // in case of Upwinding

        Laplace->LimitSize(Size, Size);

        if (PcsType == EPT_HEAT_TRANSPORT || PcsType == EPT_MASS_TRANSPORT ||
            PcsType == EPT_GAS_FLOW || PcsType == EPT_MULTI_COMPONENTIAL_FLOW ||
            PcsType == EPT_THERMAL_NONEQUILIBRIUM || PcsType == EPT_TES)
        {
    ...
    else
    {
        ...
        if (PcsType == EPT_MASS_TRANSPORT || PcsType == EPT_HEAT_TRANSPORT ||
            PcsType == EPT_MULTI_COMPONENTIAL_FLOW ||
            PcsType == EPT_THERMAL_NONEQUILIBRIUM || PcsType == EPT_TES)
        {
...
void CFiniteElementStd::SetMaterial(int /*phase*/)
{
    ...
    if (pcs->type == 1212 || pcs->type == 1313 || pcs->type == 42 ||
        (pcs->getProcessType() == FiniteElement::HEAT_TRANSPORT &&
         (PCSGet("PS_GLOBAL") || PCSGet("RICHARDS_FLOW"))))
    {
        FluidProp = MFPGet("LIQUID");
        FluidProp->Fem_Ele_Std = this;
        // FluidProp = mfp_vector[0];
...
swold for surface structure Programing: 06/2005 MB Implementation 04/2007 JOD
modifications
**************************************************************************/
void CFiniteElementStd::CalcOverlandNLTERMS(double* haa, double* haaOld,
                                            double* swval, double* swold)
{
...
/**************************************************************************
   FEMLib-Method:
   Task: Calculates consitutive relationships for CVFEM Overland Flow -> swval,
swold for surface structure Programing: 06/2007 JOD implementation
**************************************************************************/
void CFiniteElementStd::CalcOverlandNLTERMSRills(double* haa, double* haaOld,
                                                 double* swval, double* swold)
{
...
/**************************************************************************
   FEMLib-Method:
   Task: Calculates consitutive relationships for CVFEM Overland Flow -> swval,
swold for channel Programing: 06/2007 JOD Implementation
**************************************************************************/
void CFiniteElementStd::CalcOverlandNLTERMSChannel(double* haa, double* haaOld,
                                                   double* swval, double* swold)
{
    double WDepth[4], WDepthOld[4];
    double eps = MediaProp->rill_epsilon;
                                                ...
                                                double* ckwr, int* iups)
{
    ...
    else
    {
        if (MediaProp->channel == 1)
            *ckwr =
                flow_depth *
                pow(flow_depth * width / (2 * flow_depth + width), depth_exp);
        else
...
/**************************************************************************
   ...
   06/2007 JOD Implementation
**************************************************************************/
void CFiniteElementStd::CalcOverlandResidual(double* head, double* swval,
                                             double* swold, double ast,
                                             double* residual, double** amat)
{
    double sum;
    ...
    /* if(MediaProp->channel ==1){ // channel, JOD removed, don't know what it
       was for astor[0] = swval[0] * ast; astor[1] = swval[1] * ast; rhs[0] =
       swold[0] * ast * HaaOld[0]; // swval ????? rhs[1] = swold[1] * ast *
       HaaOld[1]; // swval ?????
       }
     */
...
/**************************************************************************
   ...
   06/2007 JOD Implementation
**************************************************************************/
double CFiniteElementStd::CalcOverlandJacobiNodes(int i, int j, double* head,
                                                  double* headKeep, double akrw,
                                                  double axx, double ayy,
                                                  double** amat, double* sumjac)
{
...
**************************************************************************/
void CFiniteElementStd::CalcOverlandCoefficients(double* head, double* axx,
                                                 double* ayy, double* ast)
{
...
/**************************************************************************
   ...
   08/2006 JOD Implementation
**************************************************************************/
void CFiniteElementStd::CalcOverlandCoefficientsQuad(double* head, double* axx,
                                                     double* ayy, double* ast)
{
...
/**************************************************************************
   ...
   08/2006 JOD Implementation
**************************************************************************/
void CFiniteElementStd::CalcOverlandCoefficientsTri(double* head, double* axx,
                                                    double* ayy, double* ast)
{
    double x2, x3, y2, y3;
    ...
    switch (PcsType)
    {
        ...
        case EPT_TWOPHASE_FLOW:  // Two-phase flow
            ...
            if (pcs->pcs_type_number == 0)
            {
                ...
                // version in case "L". Se = 1/M = poro/Kf + (alpha-poro)/Ks
                const double rho_val = FluidProp->Density();
            ...
            // dimensional elements in seperate functions that show
            // inconsistency in handling geo_area. For example, geo_area is
            // never used for 3D even in Old RockFlow.
            break;
        ...
        case EPT_MASS_TRANSPORT:  // Mass transport //SB4200
        {
            ...
            const double tr_phase =
                cp_vec[this->pcs->pcs_component_number]->transport_phase;
            // Multi phase transport of components
            const double saturation = PCSGetEleMeanNodeSecondary_2(
                Index, pcs->flow_pcs_type, "SATURATION1", 1);
...
void CFiniteElementStd::CalCoefMassMCF()
{
    ...
    MassMatrixElements[nDF + 1] =
        poro * rho * FluidProp->SpecificHeatCapacity(arg_PV) +
        (1.0 - poro) * SolidProp->Density(0) * SolidProp->Heat_Capacity();
    if (FluidProp->cmpN > 0)
    {
        for (in = 0; in < nDF - 2; in++)
            retardation_factore[in] =
                1.0 + (1.0 - poro) * SolidProp->Density(0) * FluidProp->Kd[in] *
                          pow(poro, -1.0);
        for (in = 2; in < nDF; in++)
            MassMatrixElements[in] = poro * rho * FluidProp->drhodX(in, arg_PV);
        for (in = 2; in < nDF; in++)
            MassMatrixElements[(nDF + 1) * in] =
                poro * retardation_factore[in - 2] * rho;
    }
    ...
    switch (PcsType)
    {
        ...
        case EPT_MASS_TRANSPORT:  // Mass transport //SB4200
        {
            ...
            porval0 = MediaProp->Porosity(
                Index,
                pcs->m_num->ls_theta);  // get porosity..this is the "old"
                                        // behaviour without GEMS coupling
...
void CFiniteElementStd::CalCoefLaplace(bool Gravity, int ip)
{
    ...
    switch (PcsType)
    {
        ...
        case EPT_MASS_TRANSPORT:  // Mass transport
            mat_fac =
                1.0;  // MediaProp->Porosity(Index,pcs->m_num->ls_theta); //
                      // porosity now included in MassDispersionTensorNew()
        ...
        case 0:
        {
            ...
            if (!Gravity)
            {
                ...
                if (diffusion)
                {
                    ...
                    D_gw = tort * rho_g * Mw * GasProp->molar_mass * M_g * M_g /
                           rhow;
...
void CFiniteElementStd::CalCoefLaplaceMCF(int ip)
{
    ...
    if (FluidProp->cmpN > 0)
    {
        for (int CIndex = 2; CIndex < FluidProp->cmpN + 2; CIndex++)
        {
            ...
            for (size_t k = 0; k < dim; k++)
            {
                LaplaceMatrixElements[(nDF + 1) * CIndex][k * dim + k] =
                    tensor[k * dim + k] * rho;
            }
        }
    }
...
void CFiniteElementStd::CalCoefLaplacePSGLOBAL(bool Gravity, int dof_index)
{
    ...
    switch (dof_index)
    {
        case 0:
            ...
            // Assemble_Gravity, no need of any code for water phase here.
            for (size_t i = 0; i < dim * dim; i++)
                mat[i] = tensor[i] * mat_fac * time_unit_factor;
            break;
        case 1:
        ...
        case 2:
            ...
            // here. However, the default water phase density should be canceled
            // out simultaneously.
            if (Gravity)
...
void CFiniteElementStd::UpwindUnitCoord(int p, int point, int ind)
{
    ...
    // v); v[0] = gp_ele->Velocity(0, point); v[1] = gp_ele->Velocity(1, point);
    ...
    switch (eletyp)
    {
        ...
        case 3:  // Hexahedra
        {
            ...
            if (upwind_meth == 1)
            {
                ...
                *supw = us + scale * alpha[1];  // scale h?gt hier ja nur von
                                                // dem letzten if ab..
...
void CFiniteElementStd::CalCoefAdvectionMCF()
{
    ...
    if (FluidProp->mu_JT == "ON")
        AdvectionMatrixElements[nDF] =
            1.0 - arg_PV[1] * FluidProp->drhodT(arg_PV);  // JOD
    AdvectionMatrixElements[nDF + 1] =
        rho * FluidProp->SpecificHeatCapacity(arg_PV);
    if (FluidProp->cmpN > 0)
        for (i = 2; i < nDF; i++)
...
void CFiniteElementStd::CalcMass()
{
    ...
    if (PcsType == EPT_TWOPHASE_FLOW)
    {
        if (upwind_method > 0)
        {
            ...
            // gravity call to his function here is also required for upwinding
            // in CalcCoefLaplace
...
/**************************************************************************
   ...
   last modification:
**************************************************************************/
void CFiniteElementStd::UpwindSummandMass(const int gp, int& gp_r, int& gp_s,
                                          int& gp_t, double* alpha,
                                          double* summand)

{
    //
    // GetGaussData(gp, gp_r, gp_s, gp_t); // this sets unit[] to standard
    // values Already computed. WW //GradShapeFunction(dshapefct, unit);
    ...
    switch (eletyp)
    {
        ...
        case MshElemType::PRISM:  // Prism
        {
            ...
            // mathlib.cpp, nicht verfügbar? u2 = MXPGaussPktTri(nGauss,gp_r,1);
...
void CFiniteElementStd::CalcMassMCF()
{
    ...
    {
        fkt =
            GetGaussData(gp, gp_r, gp_s,
                         gp_t);  // Compute Jacobian matrix and its determinate
        ...
        for (in = 0; in < nDF; in++)
        {
            ...
            for (jn = 0; jn < nDF; jn++)
            {
                ...
                for (i = 0; i < nnodes; i++)
                {
                    for (j = 0; j < nnodes; j++)
                        (*Mass2)(i + ish, j + jsh) +=
                            fkt * MassMatrixElements[in * nDF + jn] *
...
void CFiniteElementStd::CalcLaplace()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        getShapefunctValues(
            gp, 1);  // For thoese used in the material parameter caculation
        // Calculate mass matrix
        // The following "if" is done by WW
        if (PcsType == EPT_GROUNDWATER_FLOW &&
            MediaProp->unconfined_flow_group == 1 &&
            ...
            for (i = 0; i < nnodes; i++)
            {
                for (j = 0; j < nnodes; j++)
                {
                    for (size_t k = 0; k < dim; k++)
                    {
                        (*Laplace)(i + ish, j + ish) +=
                            fkt *
                            LaplaceMatrixElements[in * nDF + in][dim * k + k] *
...
void CFiniteElementStd::CalcAdvection()
{
    ...
    //    _ctx_ = true;
    //    porosity =
    //    this->MediaProp->Porosity(Index,this->pcs->m_num->ls_theta);
    //  }                                                  //18.02.2008,
    //  04.09.2008 WW
    //}
        ...
        for (std::size_t i(0); i < connected_nodes.size(); i++)
        {
            switch (coordinate_system)
            {
                case 10:  // only x-direction
                    nodal_vel[0][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVx);
                    break;
                case 11:  // only y-direction
                    nodal_vel[0][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVy);
                    break;
                case 12:  // only z-direction
                    nodal_vel[0][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVz);
                    break;
                case 21:  // x & y direction
                    nodal_vel[0][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVx);
                    nodal_vel[1][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVy);
                    break;
                ...
                case 32:  // x, y & z direction
                    ...
                    nodal_vel[1][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVy);
                    nodal_vel[2][i] = pcs_fluid_momentum->GetNodeValue(
                        connected_nodes[i], pcs_fluid_momentum->_idxVz);
                    break;
                default:
                    std::cout
                        << "  Invalid coordinate_system: " << coordinate_system
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        getGradShapefunctValues(
            gp, 1);  // Linear interpolation function....dNJ-1....var dshapefct
            ...
            for (jn = 0; jn < nDF; jn++)
            {
                ...
                for (i = 0; i < nnodes; i++)
                {
                    for (j = 0; j < nnodes; j++)
                    {
                        for (size_t k = 0; k < dim; k++)
                        {
                            (*Advection)(i + ish, j + jsh) +=
                                fkt * AdvectionMatrixElements[in * nDF + jn] *
                                shapefct[i] * vel[k] *
                                dshapefct[k * nnodes + j];
                        }
                    }
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        for (in = 0; in < nDF; in++)
        {
            ...
            for (i = 0; i < nnodes; i++)
            {
                ...
                {
                    (*Content)(i + ish, j + ish) +=
                        fkt * ContentMatrixElements[in * nDF + in] *
                        shapefct[i] * shapefct[j];
                }
            }
        }
    ...
    if (FluidProp->cmpN > 0)
    {
        ...
        for (in = 2; in < nDF; in++)
            ContentMatrixElements[(nDF + 1) * in] =
                poro * rho * retardation_factore[in - 2] *
...
void CFiniteElementStd::CalcRHS_by_ThermalDiffusion()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        drdT =
            (FluidProp->vaporDensity_derivative(TG) * humi -
...
void CFiniteElementStd::Assemble_Gravity()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        //---------------------------------------------------------
        getGradShapefunctValues(gp, 1);  // Linear interpolation function
        getShapefunctValues(gp,
                            1);  // Moved from CalCoefLaplace(). 12.3.2007 WW
        // Material
...
void CFiniteElementStd::Assemble_GravityMCF()
{
    ...
    if ((coordinate_system) % 10 == 2 && (!HEAD_Flag))
    {
        ...
        if (dim == 3 && ele_dim == 1 &&
            MediaProp->geo_inclination > 0)  // 1D fractured media
        {
            gravity_vector[0] = gravity_constant *
                                cos(PI * MediaProp->geo_inclination / 180) *
                                sin(PI * MediaProp->geo_inclination / 180);
            gravity_vector[dim - 1] =
                gravity_constant * sin(PI * MediaProp->geo_inclination / 180) *
                sin(PI * MediaProp->geo_inclination / 180);
        }
    }
    getShapeFunctionCentroid();
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        {
            for (in = 0; in < nDF; in++)
                arg_PV[in] = pcs->GetNodeValue(nodes[j],
                                               idxMCF[in + nDF]);  // current PV
            //
            rho = FluidProp->Density(arg_PV);
            //
            for (i = 0; i < nnodes; i++)
            {
                for (size_t k = 0; k < dim; k++)
                {
                    NodalVal[i] -= fkt * mat_fac * tensor[dim * k + dim - 1] *
                                   dshapefct[k * nnodes + i] * shapefct[j] *
...
void CFiniteElementStd::Assemble_Gravity_Multiphase()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        getGradShapefunctValues(gp, 1);  // Linear interpolation function
        getShapefunctValues(gp,
                            1);  // Moved from CalCoefLaplace(). 12.3.2016 WW
        // Material
        // PCH
        ...
        if (pcs->pcs_type_number == 0)  // if the pressure equation,
        {
            ...
            for (int p = 0; p < numOfPhases; ++p)
            {
                ...
                for (ii = 0; ii < dof_n; ii++)
                {
                    ...
                    // flow. This is not right for two phase gravity terms,
                    // because the equation is not summable this way. It should
                    // be seperated.
                    if (dof_n == 1)
...
#ifdef OGS_USE_CVODE
/**
 * @brief Wrapper function to interface conversion_rate with SUNDIALS CVode
 * solver
 */
int cvRhsFn_conversion_rate(realtype t, N_Vector y, N_Vector ydot,
                            void* user_data)
{
 ...
 */
void cvode_conversion_rate(const double y_ini, double delta_t,
                           conversion_rate* conv_rate, double& y_fin,
                           double& dydt_fin)
{
...
void CFiniteElementStd::CalcSolidDensityRate()
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        // Compute geometry
        //---------------------------------------------------------
        // ComputeGradShapefct(1);                  // Linear interpolation
        // function
        ...
        const double T_g =
            time_interpolate(NodalVal_t0, NodalVal_t1, theta, this);
        const double w_mf =
            time_interpolate(NodalVal_X0, NodalVal_X1, theta, this);
        double T_s;
        ...
        {
            if (this->SolidProp->getSolidReactiveSystem() ==
                FiniteElement::SINUSOIDAL)
            {  // For Benchmarks
            ...
            {  // Fuer CaOH2 im Moment
                ...
                pcs->m_conversion_rate->update_param(
                    T_s, T_g, p_g / 1.0e5, w_mf, gp_ele->rho_s_prev[gp],
                    1.0 - poro, delta_t, SolidProp->getSolidReactiveSystem());

#ifdef OGS_USE_CVODE
                const double xv_NR =
                    SolidProp->non_reactive_solid_volume_fraction;
                const double rho_NR = SolidProp->non_reactive_solid_density;
                double y_new, y_dot_new;
                cvode_conversion_rate(
                    (gp_ele->rho_s_prev[gp] - xv_NR * rho_NR) / (1.0 - xv_NR),
...
#else
                std::cout << "Error: CMake option OGS_USE_CVODE needs to be "
                             "set to solve this process type!"
                          << std::endl;
                exit(1);
#endif
            }
...
void CFiniteElementStd::Cal_Velocity()
{
    ...
    // conflict with transport calculation, needs velocities Loop over Gauss
    // points
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        {  // JOD 2014-11-10
            if ((k == 2 && (!HEAD_Flag) && FluidProp->CheckGravityCalculation())||(pcs->YDEPTH))//YDEPTH allowed CMCD
            {
                if (PcsType == EPT_THERMAL_NONEQUILIBRIUM || PcsType == EPT_TES)
                {
                    const double theta = pcs->m_num->ls_theta;
                    eos_arg[0] =
                        time_interpolate(NodalVal0, NodalVal1, theta, this);
                    eos_arg[1] =
                        time_interpolate(NodalVal_t0, NodalVal_t1, theta, this);
                    eos_arg[2] =
                        time_interpolate(NodalVal_X0, NodalVal_X1, theta, this);
                ...
                {
                    vel[dim - 1] +=
                        coef;  // NW local permeability tensor is already
                               // transformed to global one in CalCoefLaplace()
                    if (PcsType == EPT_MULTIPHASE_FLOW ||
                        PcsType == EPT_PSGLOBAL)
                    {
                        for (size_t i = 0; i < dim; i++)
                ...
                else
                {
                    ...
                    else if (PcsType == EPT_PSGLOBAL)  // PCH 05.2009
                    {
                        ...
                        vel[dim - 1] +=
                            coef;  // CB I think this should be added
            ...
            for (size_t i = 0; i < dim; i++)
            {
                for (size_t j = 0; j < dim; j++)
                    ...
                    {  // TN otherwise wrong velocity
                        tmp_gp_velocity(i, gp) -= mat[dim * i + j] /
                                                  FluidProp->Density(eos_arg) *
                                                  vel[j] / time_unit_factor;
                    }
                    else
                    {
                        tmp_gp_velocity(i, gp) -=
        ...
        else  // 02.2010. WW
        {
            for (size_t i = 0; i < dim; i++)
            {
                ...
                if (PcsType == EPT_HEAT_TRANSPORT ||
                    PcsType == EPT_MASS_TRANSPORT)  //  // JOD 2014-11-10
                    gp_ele->TransportFlux(i, gp) = 0;
#endif

                for (size_t j = 0; j < dim; j++)
                {
//              gp_ele->Velocity(i, gp) -= mat[dim*i+j]*vel[j];  // unit as that
//              given in input file
// SI Unit
#ifdef USE_TRANSPORT_FLUX
                    if (PcsType == EPT_HEAT_TRANSPORT ||
                        PcsType == EPT_MASS_TRANSPORT)  //  // JOD 2014-11-10
                        gp_ele->TransportFlux(i, gp) -=
                            mat[dim * i + j] * vel[j] / time_unit_factor;
                    else
#endif
                        tmp_gp_velocity(i, gp) -=
        ...
        for (j = 0; j < nnodes; j++)
        {
            ...
            for (size_t k = 0; k < dim; k++)
            {
                gp_ele->Velocity(k, gp) -=
                    tensor[dim * k + k] *
                    (dshapefct[k * nnodes + j] *
                         pcs->GetNodeValue(nodes[j], idxMCF[0 + nDF]) +
...
void CFiniteElementStd::Cal_GP_Velocity_FM(int* i_ind)
{
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        /*        // Write out differences:
           ...
           vel_g_old[i_dim]-vel_g[i_dim] << "  "; cout << "\n";
             }
         */
...
/***************************************************************************
 ...
 of the element center of gravity velocity for upwinding
                                               ...
                                               int phase_index, int dimension)
{
    ...
    // gp_ele->Velocity(dimension, 4)
    // << " " << gp_ele->Velocity(dimension, 5) << " " <<
    // gp_ele->Velocity(dimension, 6) << " " << gp_ele->Velocity(dimension, 7)
    // << " " << gp_ele->Velocity(dimension, 8) << " " <<
    // gp_ele->Velocity(dimension, 9)
    // << " " << gp_ele->Velocity(dimension, 10) << " " <<
    // gp_ele->Velocity(dimension,11) << " " << gp_ele->Velocity(dimension, 12)
    // << " " << gp_ele->Velocity(dimension, 13) << " " <<
    // gp_ele->Velocity(dimension, 14) << "\n";
...
{
    static double temp_vel_old[3] = {0.0, 0.0, 0.0},
                  temp_vel[3] = {0.0, 0.0, 0.0};
    ...
    // WTP: InterpolateDataFromFacesToNodes2() vs
    // InterpolateDataFromFacesToNodes()?
    this->pcs->EclipseData->InterpolateDataFromFacesToNodes(
        this->Index, NodalVal, NodalVal1, NodalVal2, phase_index);
    ...
    for (gp = 0; gp < nGaussPoints; gp++)
    {
        ...
        if (output_average == true)  // WW
        {
            for (size_t i_dim = 0; i_dim < dim; i_dim++)
                // average value of all Gauss points
                value_old[i_dim] =
                    value_old[i_dim] + temp_vel_old[i_dim] / nGaussPoints;
            for (size_t i_dim = 0; i_dim < dim; i_dim++)
                // average value of all Gauss points
...
void CFiniteElementStd::AssembleRHS(int dimension)
{
    ...
    // destruction nicely when this local function is done so that I don't
    // bother with memory leak.
    ...
    // Liquid_Flow Thus, it needs one more switch to tell Liquid_Flow and
    // Groundwater_Flow.
...
void CFiniteElementStd::AssembleParabolicEquation()
{
    ...
    if (PcsType == EPT_MULTI_COMPONENTIAL_FLOW || PcsType == EPT_TES ||
        PcsType == EPT_THERMAL_NONEQUILIBRIUM)
    {
        (*Advection) = 0.0;
        (*Content) = 0.0;
    ...
    *AuxMatrix1 -= *AuxMatrix;
    ...
    if (PcsType == EPT_MULTI_COMPONENTIAL_FLOW)  // For DOF>1: 27.2.2007 WW
    {
        for (int in = 0; in < pcs->dof; in++)
        {
            for (int i = 0; i < nnodes; i++)
            {
                NodalVal0[i + in * nnodes] =
                    pcs->GetNodeValue(nodes[i], idxMCF[in]);
...
//----------------------------------------------------------------------
    ...
    *AuxMatrix1 += *AuxMatrix;
    ...
    for (int i = 0; i < nnodes; i++)
        (*RHS)[i + LocalShift] +=
            NodalVal[i];  // RHS is later added into a global RHS in
                          // add2GlobalMatrixII()
...
equation 01/2010 NW Steady state
**************************************************************************/
void CFiniteElementStd::AssembleMixedHyperbolicParabolicEquation()
{
    ...
    //  MediaProp->NonlinearFlowFunction(Index,unit,theta); double
    //  non_linear_function_t0   = 1.0; //OK
    ...
    else
    {
        ...
        *StiffMatrix += *AuxMatrix;
        ...
        // problems ( [C] + beta1*dt [K] ) dp  for dynamic problems
...
#if !defined(USE_PETSC)  // || defined(other parallel libs)//03~04.3012. WW
    if (PcsType == EPT_MULTIPHASE_FLOW ||
        PcsType == EPT_PSGLOBAL)  // For DOF>1: 03.03.2009 PCH
    {
    ...
    switch (PcsType)
    {
        ...
        case EPT_MULTI_COMPONENTIAL_FLOW:
            for (int i = 0; i < nnodes; i++)
            {
                ...
                {
                    NodalValue[j][i] =
                        pcs->GetNodeValue(nodes[i], idxMCF[j + pcs->dof]);
                    NodalVal1[i + j * nnodes] =
                        pcs->GetNodeValue(nodes[i], idxMCF[j + pcs->dof]);
                    NodalVal0[i + j * nnodes] =
                        pcs->GetNodeValue(nodes[i], idxMCF[j]);
                }
            }
            break;
        case EPT_THERMAL_NONEQUILIBRIUM:
...
void CFiniteElementStd::Assembly()
{
    ...
    switch (PcsType)
    {
        ...
        case EPT_RICHARDS_FLOW:  // Richards flow
            ...
            break;
        //....................................................................
        case EPT_FLUID_MOMENTUM:  // Fluid Momentum - Assembly handled in
                                  // Assembly in Fluid_Momentum file
            break;
        //....................................................................
        case EPT_GAS_FLOW:  // Air (gas) flow
...
**************************************************************************/
void CFiniteElementStd::ExtropolateGauss(MeshLib::CElem& elem,
                                         CRFProcess* m_pcs, const int idof)
{
    MeshElement = &elem;
    //
    ...
    MeshLib::CElem& elem, CRFProcess* m_pcs)
{
    ...
    const int idx_nodal_solid_density =
        m_pcs->GetNodeValueIndex("SOLID_DENSITY_N");
...
void CFiniteElementStd::CalcSaturation(MeshLib::CElem& elem)
{
    ...
    for (int i = 0; i < nnodes; i++)
    {
        ...
        if (MediaProp->permeability_saturation_model[0] == 10 &&
            eS < MediaProp->capillary_pressure_values
                     [1])  // MW: limit to non-negative saturation for stability
                           // in unconfined gw
...
ElementValue::ElementValue(CRFProcess* m_pcs, CElem* ele) : pcs(m_pcs)
{
    ...
    if (pcs->getProcessType() == FiniteElement::TNEQ ||
        pcs->getProcessType() == FiniteElement::TES)
    {
    ...
    switch (dof_index)
    {
        ...
        case 2:
            ...
            rho_ga = GasProp->Density(
                dens_arg);  // AKS SEP 2010
                            // //(PG2-p_gw)*GasProp->molar_mass/(FluidConstant::GasConstant()*TG);
            rho_g = rho_ga + rho_gw;
            // 1/Mg
            M_g = (rho_gw / Mw + rho_ga / GasProp->molar_mass) / rho_g;
            D_gw = tort * rho_g * Mw * GasProp->molar_mass * M_g * M_g / rhow;
            val = D_gw * drho_gw_dT * SpecificGasConstant::WaterVapour * TG /
                  PG2 * time_unit_factor;
            break;
        case 3:
            //---------------------------------------------------------------
            //
            ...
            // From grad (p_gw/p_g)
            val = -D_ga * drho_gw_dT * SpecificGasConstant::WaterVapour * TG /
                  PG2 * time_unit_factor;
...
/***************************************************************************
 ...
 nv.Nabla p Programming: 05/2010   AKS
 **************************************************************************/
...
double CFiniteElementStd::CalCoef_RHS_HEAT_TRANSPORT2(int dof_index)
{
    ...
    if (MediaProp->evaporation == 647)
        H_vap =
            -2257000;  // pow((Tc - TG),0.38)*2.5397E+5;//It is specific you can
                       // change thi value as you chaning fluid from water
    for (size_t i = 0; i < dim * dim; i++)
        mat[i] = 0.0;
    switch (dof_index)
    {
...
#if defined(USE_PETSC)  // || defined(other parallel libs)//07.3012. WW
            NodalVal0[i + nnodes] =
                atol + rtol * max(fabs(pcs->GetNodeValue(nodes[i], idxp21)),
...
#if defined(USE_PETSC)  // || defined(other parallel libs)//07.3012. WW
            NodalVal0[i + nnodes] =
                atol + rtol * max(fabs(pcs->GetNodeValue(nodes[i], idxSn1)),
                                  fabs(x_n[nodes[i] * dof_n + 1]));
#else
            NodalVal0[i + nnodes] =
                atol + rtol * max(fabs(pcs->GetNodeValue(nodes[i], idxSn1)),
...
// CB _ctx_ CB_merge_0513
/*void CFiniteElementStd::Set_ctx_(long ele_index, double val, int gaussp, int
i_dim){
