...
public:
    ...
    void CO2_H2O_NaCl_VLE_isobaric(double T, double P, Phase_Properties& vapor,
    ...
    std::size_t
        number_of_steady_st_nodes;  // number of steady sources term entries.
    ...
    void CalGPVelocitiesfromECLIPSE(std::string path, int timestep,
                                    int phase_index, std::string phase);
    std::string
    ...
    bool SaveEclipseDataFiles;  // WTP: save Eclipse data input files for
                                // benchmarking on systems with no Eclipse
                                // licenses; use in combination with
                                // Precalculated Files
    std::string
        simulator_well_path;  // path to well schedule ( *.well), with extension
    // SB redo wtp
    std::string
        dissolved_co2_pcs_name;  // Keyword DISSOLVED_CO2_PCS_NAME; Name of
                                 // MASS_TRANSPORT Process which is used to
                                 // store total dissolved CO2 from ECLIPSE
    ...
    /*double pcs_secondary_function_time_history[PCS_NUMBER_MAX];//CMCD for
       analytical solution double
       pcs_secondary_function_value_history[PCS_NUMBER_MAX];//CMCD for
       analytical solution void Set_secondary_function_time_history(const int
       index, double value)
       {pcs_secondary_function_time_history[index]=value;}//CMCD for analytical
       solution void Set_secondary_function_value_history(const int index,
       double value) {pcs_secondary_function_value_history[index]=value;}//CMCD
       for analytical solution double Get_secondary_function_time_history(const
       int index){return pcs_secondary_function_time_history[index];}//CMCD for
       analytical solution double Get_secondary_function_value_history(const int
       index){return pcs_secondary_function_value_history[index];}//CMCD for
       analytical solution*/
    // Configuration 2 - ELE
...
// Configuration 1 - NOD
    ...
    double CalcIterationNODError(FiniteElement::ErrorMethod method,
                                 bool nls_error, bool cpl_error = false);  // OK
    ...
    virtual void
    GlobalAssembly();  // Make as a virtual function. //10.09.201l. WW
...
// WW void CheckBCGroup(); //OK
    ...
    void CalcELEMassFluxes(const GEOLIB::Polyline* const ply,
                           std::string const& NameofPolyline, double* result);
    double TotalMass[10];  // Necessary for the output of mass fluxes over
                           // polylines, BG 08/2011
    std::vector<std::string>
        PolylinesforOutput;  // Necessary for the output of mass fluxes over
                             // polylines, BG 08/2011
    ...
    void Calc2DElementGradient(MeshLib::CElem* ele,
                               double ElementConcentration[4], double* grad);
    // NEW
    ...
    bool isPointInExcavatedDomain(double const* point,
    ...
    /// Initialize the unknowns of equations with existing solutions within a
    /// index range.
...
private:
     ...
     * all COutput objects (in the vector _nod_value_vector) for the values
     * PERMEABILITY_X1 and POROSITY
     */
    void configMaterialParameters();
    // method to check on constrained source terms
    bool checkConstrainedST(std::vector<CSourceTerm*>& st_vector,
                            CSourceTerm const& st, CNodeValue const& st_node);
    // method to check on constrained boundary conditions
    bool checkConstrainedBC(CBoundaryCondition const& bc,
                            CBoundaryConditionNode& bc_node, double& bc_value);
    std::valarray<double> getNodeVelocityVector(const long node_id);
    double calcPressureFromHead(CBoundaryCondition const& bc,
                                std::size_t node_number, double pressure_value,
                                double const& time_fac = 1,
                                double const& fac = 1);
    double calcHeadFromPressure(CBoundaryCondition const& bc,
                                std::size_t node_number, double pressure_value,
                                double const& time_fac = 1,
                                double const& fac = 1);

    double evaluteSwitchBC(CBoundaryCondition const& bc,
...
/**
 * Function searches in the global pcs_vector for a process with the process
 * type pcs_type.
...
// ToDo
// SB
extern double PCSGetNODConcentration(long index, long component,
                                     long timelevel);
// SB
extern void PCSSetNODConcentration(long index, long component, long timelevel,
                                   double value);
...
extern double PCSGetEleMeanNodeSecondary(long index,
                                         ...
                                         int timelevel);
// CB
extern double PCSGetEleMeanNodeSecondary_2(long index, int pcsT,
                                           const std::string& var_name,
                                           int timelevel);
...
extern void initializeConstrainedProcesses(
