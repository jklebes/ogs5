...
{
    delete _distances;
    for (size_t i = 0; i < this->_weather_stations.size();
         i++)  // KR / NB clear climate data information
    ...
    while (!new_keyword)
    {
        ...
        if (line_string.find("$DIS_TYPE") != std::string::npos)
        {
            ...
            if (line_string.find("CONDITION") != std::string::npos)
            {
                ...
                //            in.str(GetLineFromFile1(st_file));
                in.str(readNonBlankLineFromInputStream(*st_file));
                in >> _coup_leakance >> st_rill_height >> coup_given_value >>
                    coup_residualPerm;
                in.clear();
            }  // 05.09.2008 WW
            else
        ...
        if (line_string.find("$DISTRIBUTE_VOLUME_FLUX") !=
            std::string::npos)  //  JOD 5.3.07
        {
            in.clear();
        ...
        if (line_string.find("$EXPLICIT_SURFACE_WATER_PRESSURE") !=
            std::string::npos)
        {  // JOD 5.3.07
            in.clear();
        ...
        if (line_string.find("$AIR_BREAKING") !=
            std::string::npos)  // JOD 5.3.07
        {
            in.clear();
            in.str(readNonBlankLineFromInputStream(*st_file));
            in >> air_breaking_factor >> air_breaking_capillaryPressure >>
                air_closing_capillaryPressure;
            continue;
        }
...
void CSourceTerm::ReadDistributionType(std::ifstream* st_file)
{
        ...
        this->getProcessDistributionType() == FiniteElement::CONSTANT_GEO)
    {
        ...
        in.clear();
        if (geo_node_value == .0)
            std::cout << "Warning in CSourceTerm::ReadDistributionType (): "
                         "Zero is set to the distribution type "
                      << dis_type_name
                      << ", which actually does nothing. There might be a "
                         "mistake in yoru ST file. \n";
    }
    ...
    if (dis_type_name.find("PRECIPITATION") != std::string::npos)
    {
        ...
        getline(
            ins,
            aline);  // Read the comment of the GIS headers as a dummy string.
        getline(ins, aline);
        ss.str(aline);
        for (int i = 0; i < 6; i++)
        {
    ...
    if (this->getProcessDistributionType() == FiniteElement::CLIMATE)
    {
        ...
        std::vector<GEOLIB::Point*>* stations(
            FileIO::RapidXMLInterface::readStationFile(FilePath + fname));

        const size_t nStations(stations->size());
        for (size_t i = 0; i < nStations; i++)
            _weather_stations.push_back(
                static_cast<GEOLIB::Station*>((*stations)[i]));
        ...
        if (line_string.find("#SOURCE_TERM") != std::string::npos)
        {
            ...
            if (pos != position)
            {
                if (st->getProcessPrimaryVariable() ==
                    FiniteElement::DISPLACEMENT_N)
                {
                    st->SetPressureBoundaryConditionFlag(true);
                    CSourceTerm* st2(new CSourceTerm(
                        *st));  // copies st's properties onto st2
                    st2->setProcessPrimaryVariable(
                        FiniteElement::DISPLACEMENT_X);
                    st_vector.push_back(st2);
                    st->setProcessPrimaryVariable(
                        FiniteElement::DISPLACEMENT_Y);
                }
...
element types 04/2006 OK MSH types 02/2009 WW Direct assign node source terms
**************************************************************************/
                           ...
                           std::string this_pv_name)
{
    ...
    if (m_msh)  // WW
    {
        ...
        for (long i = 0; i < no_st; i++)
        {
            ...
            // 07.01.2011. WW
            if (source_term->getProcessDistributionType() ==
            ...
            if (source_term->getProcessType() == FiniteElement::MASS_TRANSPORT)
                ...
                // != m_pcs )
                if (cp_vec[source_term->getProcessCompVecIndex()]
                        ->getProcess() !=
            ...
            if (source_term->getProcessDistributionType() ==
                ...
                source_term->getProcessDistributionType() ==
                    FiniteElement::CLIMATE)
            {  // NB For climate ST, the source terms (recharge in this case)
               // will also be assigned directly to surface
                // nodes
            ...
            if (convertProcessTypeToString(source_term->getProcessType())
                    .compare(pcs_type_name) == 0)
            {
                if (convertPrimaryVariableToString(
                        source_term->getProcessPrimaryVariable())
                        .compare(pcs_pv_name) == 0)
                    set = true;
                else if ((source_term->getProcessType() ==
                          FiniteElement::MASS_TRANSPORT) &&
                         (cp_vec[source_term->getProcessCompVecIndex()]
                              ->compname.compare(pcs_pv_name) == 0))
                    set = true;
            }

            // 05/2012 BG, it does not work for mass transport if the second
            // part with "CONCENTRATION" is not added !! problem identified by
            // GK und HS if ((convertProcessTypeToString
            // (source_term->getProcessType ()).compare(pcs_type_name) == 0)
            //   &&
            ...
            if (set)
            {
                ...
                if (source_term->getProcessDistributionType() ==
                        FiniteElement::RECHARGE ||
                    source_term->getProcessDistributionType() ==
                        FiniteElement::RECHARGE_DIRECT)  // MW
...
double AreaProjection(CEdge* edge,
                      FiniteElement::PrimaryVariable primaryVariable)
{
    ...
    edge->SetNormalVector(elemNormalVector,
                          edge_normal);  // Attenzione! This only works if nodes
                                  ...
                                  std::vector<double>& node_value_vector) const
{
    ...
    double area_projection(1.0);  // for projection of element areas for edges
    ...
    for (i = 0; i < (long)msh->edge_vector.size(); i++)
    {
        ...
        {
            area_projection =
                AreaProjection(edge, this->getProcessPrimaryVariable());
        }
...
#if defined(USE_PETSC)  // || defined (other parallel linear solver lib). //WW.
                        // 05.2013
                    if (e_nodes[0]->isNonGhost(id_act_l_max, id_l_max,
                                               id_act_h_max))
#endif
                        NVal[G2L[e_nodes[0]->GetIndex()]] += Jac * v1 / 3.0;
#if defined(USE_PETSC)  // || defined (other parallel linear solver lib). //WW.
                        // 05.2013
                    if (e_nodes[1]->isNonGhost(id_act_l_max, id_l_max,
                                               id_act_h_max))
#endif
                        NVal[G2L[e_nodes[1]->GetIndex()]] += Jac * v1 / 3.0;
#if defined(USE_PETSC)  // || defined (other parallel linear solver lib). //WW.
                        // 05.2013
                    if (e_nodes[2]->isNonGhost(id_act_l_max, id_l_max,
                                               id_act_h_max))
#endif
                        NVal[G2L[e_nodes[2]->GetIndex()]] +=
                            4.0 * Jac * v1 / 3.0;
                }
                else
                {
                    for (k = 0; k < 3; k++)  // Three nodes
                    {
#if defined(USE_PETSC)  // || defined (other parallel linear solver lib). //WW.
                        // 05.2013
                        if (!e_nodes[k]->isNonGhost(id_act_l_max, id_l_max,
                                                    id_act_h_max))
                            continue;
#endif
                        // Numerical integration
                        for (l = 0; l < 3; l++)  // Gauss points
                        {
                            ...
                            if (msh->isAxisymmetry())
                            {
                                ...
                                for (ii = 0; ii < 3; ii++)
                                    radius +=
                                        Shfct[ii] * e_nodes[ii]->getData()[0];
                                if (fabs(radius) <
                                    DBL_MIN)  // At the symmetric axis.
                                    radius = 1.0;

                                Weight *= radius;  // 2.0*pai*radius;
                            }
                            NVal[G2L[e_nodes[k]->GetIndex()]] +=
                                0.5 * (v1 + v2 + eta * (v2 - v1)) * Shfct[k] *
                                Weight;
                        }
                    }
                }
 ...
 **************************************************************************/
void CSourceTerm::FaceIntegration(CRFProcess* pcs,
                                  ...
                                  std::vector<double>& node_value_vector)
{
    ...
    if (!Const)  // Get node BC by interpolation with surface
    {
        ...
        for (j = 0; j < Size; j++)
        {
            ...
            while (p != m_surface->polyline_of_surface_vector.end())
            {
                ...
                // BC value at center is an average of all point values of
                // polygon
    ...
    for (i = 0; i < (long)msh->ele_vector.size(); i++)
    {
        ...
        // Activate all element in case that surface is defined inside the
        // domain.
    ...
    CElement* fem_assembler_linear =
        dynamic_cast<CElement*>(pcs->getLinearFEMAssembler());
        ...
        pcs->getProcessType() == FiniteElement::DEFORMATION_H2)
    {
        process::CRFProcessDeformation* dm_pcs =
            dynamic_cast<process::CRFProcessDeformation*>(pcs);
        fem_assembler_quad = dynamic_cast<CElement*>(dm_pcs->GetFEMAssembler());
    }

    CElement* fem_assembler =
        (msh->getOrder() == 1) ? fem_assembler_quad : fem_assembler_linear;
...
void CSourceTerm::DomainIntegration(
    CRFProcess* pcs, const std::vector<long>& nodes_in_dom,
    std::vector<double>& node_value_vector) const
{
    double nodesFVal[8];
    vec<CNode*> e_nodes(20);
...
/**************************************************************************
 ...
 Programing:
 01/2007 JOD Implementation
 10/2008 JOD overland node shifting for soil columns, averaging
 automatically 4.7.10 12/2012 JOD Extension to TWO_PHASE_FLOW  5.3.07
 **************************************************************************/
#if !defined(USE_PETSC) && \
    !defined(NEW_EQS)  // && defined(other parallel libs)//03~04.3012. WW
//#ifndef NEW_EQS                                   //WW. 06.11.2008
                               ...
                               CNodeValue* cnodev)
{
    ...
    if ((mesh_node_number = cnodev->msh_node_number) <
        (long)m_pcs_this->m_msh->nod_vector.size())
    {  // liquid
        mesh_node_number_conditional = cnodev->msh_node_number_conditional;
        if (m_st->getProcessType() == FiniteElement::RICHARDS_FLOW)
            gamma = mfp_vector[0]->Density() *
                    GRAVITY_CONSTANT;  // liquid pressure as a primary variable
        else if (m_st->getProcessType() == FiniteElement::MULTI_PHASE_FLOW)
            gamma =
                -mfp_vector[0]->Density() *
                GRAVITY_CONSTANT;  // capillary pressure as a primary variable
    }
    else
    {  // gas
        ...
        /*if( m_st->pcs_type_name_cond == "MULTI_PHASE_FLOW")
           mesh_node_number_conditional = cnodev->msh_node_number_conditional -
        m_pcs_this->m_msh->nod_vector.size(); else*/
        mesh_node_number_conditional = cnodev->msh_node_number_conditional;
        gamma = mfp_vector[1]->Density() *
                GRAVITY_CONSTANT;  // gas pressure as a primary variable
    }

    GetCouplingFieldVariables(m_pcs_this, m_pcs_cond, &h_this, &h_cond,
                              &h_cond_shifted, &help, &z_this, &z_cond, m_st,
                              cnodev, mesh_node_number,
                              mesh_node_number_conditional,
                              gamma);  // z_cond shifted for soil columns

    /////   relative interface permeability
    if (m_st->pcs_pv_name_cond ==
        "PRESSURE2")  // || m_st->pcs_type_name_cond == "OVERLAND_FLOW")
    {                 // gas
        CRFProcess* m_pcs_overland = NULL;
        double head_overland;
        m_pcs_overland = PCSGet(m_st->pcs_type_name_cond);
        head_overland = m_pcs_overland->GetNodeValue(
            cnodev->msh_node_number_conditional,
            m_pcs_overland->GetNodeValueIndex("HEAD") + 1);

        relPerm = m_st->GetRelativeInterfacePermeability(
            m_pcs_overland, head_overland,
            cnodev->msh_node_number_conditional);  // overland
    }
    else
    {  // liquid
        if (h_this <
            h_cond_shifted)  // flow direction from overland compartment
            relPerm = m_st->GetRelativeInterfacePermeability(
                m_pcs_cond, h_cond, cnodev->msh_node_number_conditional);
        else  // flow direction from subsurface compartment (, where now relPerm
              // = 1)
            relPerm = m_st->GetRelativeInterfacePermeability(
                m_pcs_this, h_this, cnodev->msh_node_number_conditional);
    }
    ...
    condArea =
        value * relPerm *
        m_st->getCoupLeakance();  // area * total interface permeability (m^2/s)
    ...
    if (m_st->getProcessType() == FiniteElement::MULTI_PHASE_FLOW &&
        m_st->pcs_pv_name_cond == "HEAD"                 //
        && m_st->pcs_type_name_cond == "OVERLAND_FLOW")  // ???
    {                                                    // gas pressure term
        MXInc(cnodev->msh_node_number,
              cnodev->msh_node_number + m_pcs_this->m_msh->nod_vector.size(),
              -condArea);
    ...
    if (m_st->getProcessType() == FiniteElement::GROUNDWATER_FLOW)
        m_pcs_this->SetNodeValue(
            mesh_node_number,  // for GW water balance ply / pnt
            m_pcs_this->GetNodeValueIndex("FLUX") + 1, condArea);

    if (m_st->no_surface_water_pressure)  // neglect hydrostatic surface liquid
                                          // pressure
        value = 0;
}
#endif
                               ...
                               CNodeValue* cnodev)
{
    ...
    GetCouplingFieldVariables(
        m_pcs_this, m_pcs_cond, &h_this, &h_cond, &h_this_shifted,
        // if soil column, z_this, h_this_averaged, h_this_shifted are shifted
        // to the column
    ...
    {  // flow direction from the overland compartment
        relPerm = m_st->GetRelativeInterfacePermeability(
            m_pcs_this, h_this, cnodev->msh_node_number);
        relPerm_epsilon = m_st->GetRelativeInterfacePermeability(
            m_pcs_this, h_this + epsilon,  // for jacobian
    ...
    {  // h_this_epsilon for jacobian if multiple overland nodes are coupled to
       // a soil column
    ...
    if (m_st->explicit_surface_water_pressure)
    {  // put hydrostatic surface liquid pressure explicitly in the coupling
       // flux
            ...
            for (long i = 0;
                ...
                h_this_averaged +=
                    ...
                    (m_pcs_this->GetNodeValue(
                         cnodev->msh_node_numbers_averaging[i], 0) -
                     m_pcs_this->m_msh
    ...
    value_jacobi = -condArea_epsilon * (h_cond - h_this_epsilon) +
                   value;  // it's a Newton iteration
...
double CSourceTerm::GetRelativeInterfacePermeability(CRFProcess* pcs,
                                                     double head, long msh_node)
{
    ...
    {  // flow direction from the overland compartment
        ...
        sat =
            (head - z) /
            std::max(1.e-6, st_rill_height);  // liquid content in an interface
        ...
        if (sat > 1)
            ...
            relPerm = pow(
                sat,
                2 * (1 - sat));  // interface is partially filled by a liquid

        CRFProcess* m_pcs_multiPhase = NULL;
        m_pcs_multiPhase = PCSGet("MULTI_PHASE_FLOW");

        if (relPerm == 1 &&
            pcs_pv_name_cond == "PRESSURE2")  // surface closed for gas
        {
            double capillaryPressure = m_pcs_multiPhase->GetNodeValue(0, 3);

            if (capillaryPressure > air_breaking_capillaryPressure ||
                air_breaking == true)
            {
        ...
        {  // gas
            relPerm =
                (1 - relPerm) * (1 - coup_residualPerm) + coup_residualPerm;
        }
    }  // end overland flow
    else
                              ...
                              CNodeValue* cnodev)
{
    ...
    //   double y_this =
    //   m_pcs_this->m_msh->nod_vector[cnodev->msh_node_number]->Y(); double
    ...
    if (m_st->getProcessType() == FiniteElement::OVERLAND_FLOW)
    {
        ...
        // -pressure0, 0); cond0 *=
        // mmp_vector[group]->PermeabilitySaturationFunction(sat,0);
...
// msh_node) //WW void GetRiverNODValue(double &value, CNodeValue* cnodev, const
// CSourceTerm* m_st) //WW
//{
//	double h;
...
void GetNODHeatTransfer(double& value, CSourceTerm* st, long geo_node)
{
    ...
    size_t number_of_connected_elements =
        mesh->nod_vector[geo_node]->getConnectedElementIDs().size();
    ...
    // if (mesh->isAxisymmetry() && mesh->GetMaxElementDim()!=1) //For
    // axisymmetric 2D meshes geometry area is irrelevant geo_area = 1.0;
                              ...
                              const int ShiftInNodeVector)
{
    // 05.2012. WW
    long node_id =
        m_msh->GetNODOnPNT(static_cast<const GEOLIB::Point*>(st->getGeoObj()));
    if (node_id < 0)
        return;
    ...
    {  // TN - Belegung mit Fl�chenelementen
        ...
        ////Get process type
        // CRFProcess* m_pcs_this =
        // PCSGet(convertProcessTypeToString(st->getProcessType()));
        ////Get Mesh
        // CFEMesh* mesh (m_pcs_this->m_msh);
        // long msh_ele =
        // mesh->nod_vector[nod_val->geo_node_number]->getConnectedElementIDs()[0];
        //   int group = mesh->ele_vector[msh_ele]->GetPatchIndex();

        // st->node_value_vectorArea[0] = mmp_vector[group]->geo_area;
        if (m_msh->isAxisymmetry() && m_msh->GetMaxElementDim() == 1)
            st->node_value_vectorArea[0] *=
                m_msh->nod_vector[nod_val->geo_node_number]
                    ->X();  // 2pi is mulitplicated during the integration
                            // process
    }
...
void CSourceTermGroup::SetPLY(CSourceTerm* st, int ShiftInNodeVector)
{
    ...
    if (old_ply)
    {
        ...
        if (st->isCoupled())
            SetPolylineNodeVectorConditional(st, ply_nod_vector,
                                             ply_nod_vector_cond);
...
void CSourceTermGroup::SetSFC(CSourceTerm* m_st, const int ShiftInNodeVector)
{
    ...
    if (m_sfc)
    {
        ...
        sfc_nod_vector.insert(sfc_nod_vector.begin(), sfc_node_ids.begin(),
                              sfc_node_ids.end());
        if (m_st->isCoupled())
        ...
        if (m_st->isConstrainedST())
        {
            for (std::size_t i(0); i < m_st->st_node_ids.size(); i++)
            {
                ...
                for (std::size_t i(0); i < m_st->getNumberOfConstrainedSTs();
                     i++)
...
void CSourceTermGroup::SetSurfaceNodeVector(
    GEOLIB::Surface const* sfc, std::vector<std::size_t>& sfc_nod_vector,
    const double epsilon)
{
    double computed_search_length = m_msh->getSearchLength();
    if (epsilon != -1)
    ...
    if (st->node_averaging)
    {
        if (m_msh_cond)
        {
                ...
                pcs_type_name == "MULTI_PHASE_FLOW")
            {
                ...
                assembled_mesh_node = 0;  // ply_nod_vector[0]; // JOD  carefull
                                          // !!! ply sometimes fails
                     ...
                     pcs_type_name == "GROUNDWATER_FLOW")  // JOD 4.10.01
            {
                ...
                m_msh_cond->GetNODOnPLY(
                    static_cast<const GEOLIB::Polyline*>(st->getGeoObj()),
                    ply_nod_vector_cond);
                assembled_mesh_node = 0;  // ply_nod_vector_cond[0];  // JOD
                                          // carefull !!! ply sometimes fails
                ...
                std::cout
                    << "Warning in "
                       "CSourceTermGroup::SetPolylineNodeVectorConditional - "
                       "no area assembly for "
                       "this process"
                    << "\n";
        }  // end mesh_cond
        else
        ...
        if (m_msh_cond) {
            ...
            // JOD 4.10.01
                ...
                for (size_t i = 0; i < number_of_nodes; i++)
                    ...
                    << "Warning in
CSourceTermGroup::SetPolylineNodeVectorConditional - no area assembly for this
process"
                    << "\n";
        } // end mesh_cond
        else std::cout
...
/**************************************************************************
 ...
 last modification:
 **************************************************************************/
/*void CSourceTermGroup::SetPolylineNodeValueVector(CSourceTerm* st, CGLPolyline
* old_ply, const std::vector<long>& ply_nod_vector, std::vector<long>&
ply_nod_vector_cond, std::vector<double>& ply_nod_val_vector)
{
    ...
    if (st->getProcessDistributionType() == FiniteElement::LINEAR
            || st->getProcessDistributionType() ==
FiniteElement::LINEAR_NEUMANN) { st->InterpolatePolylineNodeValueVector(old_ply,
st->DistribedBC, ply_nod_val_vector); } else if
(st->getProcessDistributionType() == FiniteElement::SYSTEM_DEPENDENT) {
...
node_on_line++; } // end nodes } // end vertices if (node_on_line == 2)
    ...
    else //WW
    {
        for (long i = 0; i < number_of_nodes; i++) {
            ...
            if (st->getProcessDistributionType() == FiniteElement::CONSTANT_GEO)
                ply_nod_val_vector[i] = st->geo_node_value / (double)
number_of_nodes; // distribute flow to nodes along polyline. To do.. 4.10.06
        }
    }
...
// 09/2010 TF
void CSourceTermGroup::SetPolylineNodeValueVector(
    CSourceTerm* st,
    ...
    std::vector<double>& ply_nod_val_vector) const
{
    ...
    else if (distype == FiniteElement::FUNCTION)  // 25.08.2011. WW
    {
        for (size_t i = 0; i < number_of_nodes; i++)
        {
            double const* const pnt(
                m_msh->nod_vector[ply_nod_vector[i]]->getData());
            ply_nod_val_vector[i] =
                st->dis_linear_f->getValue(pnt[0], pnt[1], pnt[2]);
        }
    }
    else  // WW
    {
        for (size_t i = 0; i < number_of_nodes; i++)
        {
            ...
            if (st->getProcessDistributionType() == FiniteElement::CONSTANT_GEO)
                ply_nod_val_vector[i] =
                    st->geo_node_value / (double)number_of_nodes;
        }
    }
    ...
    {  // TN - Belegung mit Fl�chenelementen
        ...
        if (m_msh->GetMaxElementDim() == 1)  // 1D  //WW MB
            st->DomainIntegration(m_pcs, ply_nod_vector,
    ...
    std::vector<double>& ply_nod_val_vector) const
{
    if (pcs_type_name == "RICHARDS_FLOW" || pcs_type_name == "MULTI_PHASE_FLOW")
    {
        ...
        {
            CRFProcess* m_pcs = PCSGet(pcs_type_name);
            st->DomainIntegration(m_pcs, ply_nod_vector_cond,
                                  ply_nod_val_vector);
        }
        else
...
/**************************************************************************
 ...
 last modification:
 **************************************************************************/
void CSourceTermGroup::SetSurfaceNodeValueVector(
    CSourceTerm* st, Surface* m_sfc, std::vector<long> const& sfc_nod_vector,
    std::vector<double>& sfc_nod_val_vector)
{
    CRFProcess* m_pcs = PCSGet(pcs_type_name);
		...
		st->getProcessDistributionType() == FiniteElement::CAUCHY)
    {
        ...
        else if (m_msh->GetMaxElementDim() ==
                 3)  // For all meshes with 3-D elements
            st->FaceIntegration(m_pcs, sfc_nod_vector, sfc_nod_val_vector);
    }  // end neumann
    else if (st->getProcessDistributionType() ==
             FiniteElement::FUNCTION)  // 25.08.2011. WW
    {
        for (size_t j = 0; j < sfc_nod_vector.size(); j++)
        {
            double const* const pnt(
                m_msh->nod_vector[sfc_nod_vector[j]]->getData());
            sfc_nod_val_vector[j] =
                st->dis_linear_f->getValue(pnt[0], pnt[1], pnt[2]);
        }
    }
}
                                              ...
                                              std::vector<double>& node_valus)
{
    ...
    for (std::size_t i = 0; i < node_ids.size(); i++)
    {
        ...
        if (!m_msh->nod_vector[id]->isNonGhost(id_act_l_max, id_l_max,
                                               id_act_h_max))
            // ghost entry, removed
 ...
 **************************************************************************/
void CSourceTermGroup::DistributeVolumeFlux(CSourceTerm* st,
                                            std::vector<long>& nod_vector,
                                            std::vector<double>& nod_val_vector)
{
    double area;
    ...
    if (st->getGeoType() == GEOLIB::POLYLINE)
        SetPolylineNodeValueVector(st, nod_vector, nod_vector,
                                   nod_val_vector_area);
    else if (st->getGeoType() == GEOLIB::SURFACE)
    {
        ...
        std::cout << "GEO_TYPE not supported in "
                     "CSourceTermGroup::DistributeVolumeFlux()"
                  << "\n";
    ...
    else
    {
        std::cout << "Using the geometric object " << st->geo_name
                  << " does not find any nearby nodes. No ST applied there!"
                  << std::endl;
    }
}
                                ...
                                int ShiftInNodeVector)
{
    ...
    for (size_t i = 0; i < number_of_nodes; i++)
    {
        ...
        if (st_vector[m_nod_val->getSTVectorGroup()]->isConstrainedST())
            st_vector[m_nod_val->getSTVectorGroup()]
                ->setConstrainedSTNodesIndex(m_nod_val->geo_node_number,
...
{
    if (getProcessDistributionType() ==
        FiniteElement::CLIMATE)  // NB for this type of ST, we assign a ST to
                                 // each node on the Mesh surface (land surface)
    {
        std::vector<double> node_area_vec;
        MshEditor::getNodeAreas(m_pcs->m_msh, node_area_vec);
        const std::vector<GEOLIB::PointWithID*>& points(
            MshEditor::getSurfaceNodes(*(m_pcs->m_msh)));
        ...
        for (size_t i = 0; i < nPoints; i++)
        {
            ...
            m_nod_val->node_value =
                std::numeric_limits<double>::min();  // values will be assigned
                                                     // in IncorporateSoureTerms
                                                     // (rf_pcs.cpp)
            m_nod_val->CurveIndex = CurveIndex;
            m_pcs->st_node_value.push_back(m_nod_val);
            m_pcs->st_node.push_back(this);
            m_pcs->m_msh->nod_vector[node_id]->patch_area =
                node_area_vec[node_id];
        }

        this->_distances =
            new MathLib::InverseDistanceInterpolation<GEOLIB::PointWithID*,
                                                      GEOLIB::Station*>(
                points, this->_weather_stations);
    }
    else  // NB this is the old version, where nodes were read from an separate
          // input file
    {
...
**************************************************************************/
std::string CSourceTerm::assignPrecipitationDirectlyOnTopSurfaceNodes(
    double current_time)
{
    ...
    if (m_pcs->st_node.size() > 0)
    {
             ...
             i++)
        {
            ...
            // st_vector). its memory cannot be released in the destructor
        }
        m_pcs->st_node.erase(
            m_pcs->st_node.begin() + m_pcs->number_of_steady_st_nodes,
            m_pcs->st_node.end() - m_pcs->number_of_steady_st_nodes);
        m_pcs->st_node_value.erase(
            m_pcs->st_node_value.begin() + m_pcs->number_of_steady_st_nodes,
...
double CSourceTerm::GetAnalyticalSolution(long location)
{
    ...
    long node_number =
        location;  // WW m_pcs->st_node_value[location]->msh_node_number;
    ...
    process_no *= 2;  // first column time, second column value, hence two
                      // columns per process;
    ...
    if ((aktueller_zeitschritt - 1) % this->resolution == 0)
    {
        ...
        ref_value =
            this->GetNodePastValueReference(node_number, (process_no / 2));
        ...
        //  if (area < 0) {//set in set source terms function, domain area = -1
        //  to start with
        if (area < DBL_MIN)
        {  // HS 04.2008
            tvol = 0.0;
            ...
            for (i = 0; i < (int)Node->getConnectedElementIDs().size(); i++)
            {
                ...
                flux_area =
                    Ele->GetFluxArea();  // Real thickness for a fracture
...
/**************************************************************************
 ...
 data of nodes last modification:
 **************************************************************************/
 ...
 **************************************************************************/
void GetCouplingFieldVariables(CRFProcess* m_pcs_this, CRFProcess* m_pcs_cond,
                               double* h_this, double* h_cond,
                               double* h_shifted, double* h_averaged,
                               double* z_this, double* z_cond,
                               CSourceTerm* m_st, CNodeValue* cnodev,
                               long msh_node_number, long msh_node_number_cond,
                               double gamma)
{
    int nidx, nidx_cond;
    ...
    *h_this = m_pcs_this->GetNodeValue(msh_node_number, nidx);
    ...
    if (m_st->pcs_pv_name_cond == "GIVEN_HEIGHT" ||
        m_st->pcs_pv_name_cond == "PRESSURE2")
    {
        *h_cond = *h_shifted =
            m_st->coup_given_value;  // coupled to fixed pressure head / or
    ...
    {
        *h_cond = *h_shifted =
            m_st->coup_given_value /
            gamma;  // coupled to fixed pressure head / or atmospheric pressure
        *z_this = *z_cond = 0;
        return;
    }
    else
        *h_cond = m_pcs_cond->GetNodeValue(msh_node_number_cond,
                                           nidx_cond);  // coupled to a liquid
    ...
    if (m_st->pcs_pv_name_cond == "PRESSURE1")
    {
        ...
        {  // capillary pressure as a primary variable in the coupled process
            double gasPressure =
                m_pcs_cond->GetNodeValue(msh_node_number_cond, 3);
    ...
    {
        if (m_st->node_averaging)
        {  // average pressures from overland / groundwater (soil column coupled
           // to multiple nodes)
            *h_shifted =
                *h_this - *z_this +
        ...
        if (m_st->pcs_type_name_cond == "GROUNDWATER_FLOW")
            h_cond = std::max(
                h_cond, z_this);  // groundwater level might not reach surface
    }                             // end overland flow
    else
    {                                               // subsurface flow
                         ...
                         double z_cond, CSourceTerm* m_st)
{
    if (m_st->getProcessType() == FiniteElement::OVERLAND_FLOW)
    {
        if (m_st->no_surface_water_pressure)
            return factor *
                   (h_cond -
                    z_cond);  // neglect hydrostatic surface liquid pressure
        else
    ...
    else
    {
        if (m_st->getProcessType() == FiniteElement::GROUNDWATER_FLOW)
        {
            ...
            return factor *
                   (h_cond -
                    z_cond);  // Richards', two-phase flow (liquid & gas)
    }
}
