...
ios::pos_type CNumerics::Read(ifstream* num_file)
{
    ...
    while (!new_keyword)
    {
        ...
        if (line_string.find("$NON_LINEAR_UPDATE_VELOCITY") !=
            string::npos)  // NW
        {
            line.str(GetLineFromFile1(num_file));
        ...
        if (line_string.find("$ADDITIONAL_NEWTON_TOLERANCES") != string::npos)
        {
            ...
            line.clear();
            std::cout << "Additional Newton tolerances have been set. Global "
                         "NR will converge if: "
                      << std::endl;
            std::cout << "  DeltaF/DeltaF0 <= " << nls_error_tolerance
                      << " (set using $NON_LINEAR_ITERATIONS)" << std::endl;
            std::cout << "  && DeltaF <= " << nls_abs_residual_tolerance
                      << std::endl;
            std::cout << "  && DeltaU <= " << nls_abs_unknown_tolerance
                      << std::endl;
            std::cout << "  && DeltaU/DeltaU0 <= " << nls_rel_unknown_tolerance
                      << std::endl;
            continue;
        }
        ...
        /*
           ...
           found while((!new_keyword)||(!new_subkeyword)||(!num_file->eof())){
...
void SetLinearSolverType(LINEAR_SOLVER* ls, CNumerics* m_num)
{
    ...
    /*
       ...
       if (!ls->lsp) {
       ls->lsp =
       GetLinearSolverProperties(MASTER_DEFAULT_LINEAR_SOLVER_PROPERTIES); if
       (ls->lsp) { ls->store = get_lsp_store(ls->lsp);
       }
       }
     */
...
/**************************************************************************/
double NUMCalcIterationError(double* new_iteration, double* old_iteration,
                             double* reference, long length, int method)
{
    static long i;
