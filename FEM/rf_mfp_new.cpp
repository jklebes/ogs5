      ...
      densityIAPWS(NULL)
{
    ...
    cal_gravity = true;
    drho_dT_unsaturated =
        false;  // considering fluid expansion due to temperature in the
                // unsaturated case? (Richards)
    // Data
...
std::ios::pos_type CFluidProperties::Read(std::ifstream* mfp_file)
{
    ...
    while (!new_keyword)
    {
        ...
        {
            in.str(GetLineFromFile1(mfp_file));
            in >> cmpN >> cmpNm1 >> cmpNm2 >> cmpNm3 >>
                cmpNm4;  // get up to four component
            in.clear();
            continue;
        }
        //....................................................................
        ...
        if (line_string.find("$COMPRESSIBILITY") != string::npos)
        {
            ...
            if (compressibility_model_pressure ==
                15)  // components constant density
            {
            ...
            // CFluidProperties::drhodT 0 incompressible 1 constant slope 2
            // slope from fct_table 3 difference quotient 4 analytical
            // derivation
        ...
        {
            drho_dT_unsaturated =
                true;  // if keyword found, drho/dT will be considered for
                       // unsaturated case (richards)
            continue;
        }
        //....................................................................
        ...
        if (line_string.find("$DENSITY") != string::npos)
        {
            ...
            if (density_model == 3)  // rho(C) = rho_0*(1+beta_C*(C-C_0))
            {
                ...
                density_pcs_name_vector.push_back(
                    "CONCENTRATION1");  // JOD 2014-11-10, consistent with
                                        // output DELTA_CONCENTRATION1
            ...
            /// Density model no. 19: Extracts fluid densities from GEM3K
            /// calculations The densites depend on the thermodynamic data base
            /// (including pressure and temperature corrections) in addition
            /// various gas/fluid models (&EOS) are defined in GEMS3K
            if (density_model ==
                19)  // KG44 get node densities from GEMS calculations
            {
            }
            if (density_model == 20)  // rho(C) = rho (p, T)*(1+beta_C*(C-C_0))
            ...
            if (density_model == 26)  // && pcs_vector[0]->getProcessType() !=
                                      // FiniteElement::TNEQ)
            {
                std::cout << "Warning: This density model requires two "
                             "components and their molar masses defined in "
                             "the mcp file!\n";
                density_pcs_name_vector.push_back("PRESSURE1");
                density_pcs_name_vector.push_back("TEMPERATURE1");
                density_pcs_name_vector.push_back("CONCENTRATION1");
        ...
        if (line_string.find("$VISCOSITY") != string::npos)
        {
            ...
            /// Viscosity model no. 19: Extracts viscosities from GEM3K
            /// calculations (not yet implemented)
            if (viscosity_model == 19)  // KG44 extract viscosity from GEMS
            {
            }
            if (viscosity_model == 26)  // && pcs_vector[0]->getProcessType() !=
                                        // FiniteElement::TNEQ)
            {
                std::cout << "Warning: This viscosity model requires two "
                             "components and their molar masses defined in "
                             "the mcp file!\n";
                viscosity_pcs_name_vector.push_back("PRESSURE1");
                viscosity_pcs_name_vector.push_back("TEMPERATURE1");
        ...
        if (line_string.find("$SPECIFIC_HEAT_CAPACITY") != string::npos)
        {
            ...
            if (heat_capacity_model == 11 || heat_capacity_model == 12 ||
                heat_capacity_model ==
                    13)  // && pcs_vector[0]->getProcessType() !=
                         // FiniteElement::TNEQ)
            {
                std::cout << "Warning: This heat capacity model requires two "
                             "components and their molar masses defined "
                             "in the mcp file!\n";
                specific_heat_capacity_pcs_name_vector.push_back("PRESSURE1");
                specific_heat_capacity_pcs_name_vector.push_back(
                    "TEMPERATURE1");
        ...
        if (line_string.find("$HEAT_CONDUCTIVITY") != string::npos)
        {
            ...
            if (heat_conductivity_model ==
                11)  // && pcs_vector[0]->getProcessType() !=
                     // FiniteElement::TNEQ)
            {
                std::cout << "Warning: This heat conductivity model requires "
                             "two components and their molar masses "
        ...
        {
            in.str(GetLineFromFile1(mfp_file));
            in >> decay_model >> lambda[0] >> lambda[1] >> lambda[2] >>
                lambda[3];
            in.clear();
        }
        //....................................................................
...
void CFluidProperties::Write(std::ofstream* mfp_file) const
{
    ...
    *mfp_file << " $PHASE_DIFFUSION"
              << "\n";
    // if(diffusion_model == 0) *mfp_file << "  " << diffusion_model << " " <<
    // dif_fct_name << "\n";
...
double CFluidProperties::Density(double* variables)
{
    ...
    if (variables)  // This condition is added by WW
    {
        ...
        {
            case 0:  // rho = f(x)
                density = GetCurveValue(density_curve_number, 0, variables[1],
                                        &gueltig);
                break;
            case 1:  // rho = const
                density = rho_0;
            ...
            case 15:  // mixture 1/rho= sum_i x_i/rho_i #p, T, x:->Amagat's law#
                for (int CIndex = 2; CIndex < cmpN + 2; CIndex++)
                    Rho +=
                        variables[CIndex] / ComponentDensity(CIndex, variables);
                density = 1 / Rho;
                break;
            ...
            case 18:  // using calculated densities at nodes from the phase
                ...
                variables[2] = phase;
                density = GetElementValueFromNodes(
                    int(variables[0]),
                    int(variables[1]),
                    int(variables[2]),
                    0);  // hand over element index, Gauss point index and phase
                         // index
                break;
            case 19:  // KG44 get the density from GEMS calculations
                // seems complicated, as we probably have to call GEMS.....or
                // take values from last GEMS calculation
                // ---> update during iterations is not possible
                //  long elem = Fem_Ele_Std->GetMeshElement()->GetIndex();
                density = 1000.0;
#ifdef GEM_REACT
                if (!Fem_Ele_Std)  // for Richards flow (when saturation is
                                   // needed initially in GEMS setup) we have to
                // make
                {
                    density = 1000.0;
                }
                ...
                {
                    // elem = Fem_Ele_Std->GetMeshElement()->GetIndex(); //kg44
                    // need element index or node index for GEMS ...currently we
                    // stick to arithmetric average of node data (even for gauss
                    // points!)
                    density = m_vec_GEM->REACT_GEM::FluidDensity(
                        long(variables[0]),
                        int(variables[1]));  // hand over element index and
                                             // gauss point index
                    // here we can interpolate values from nodes to elemnt or to
                    // gauss points
                }
#endif
                // insert call for GEMS densities..
                break;
            case 20:  // rho(p,T, C) for water, range p < 100 MPa, 0 <= T <= 350
                      // °C   Magri GFZ thesis

                pressure = variables[0] / 1e5;
                density = 9.99792877961606e+02 +
                          5.07605113140940e-04 * max(pressure, 0.0) -
                          5.28425478164183e-10 * pow(max(pressure, 0.0), 2.) +
                          (5.13864847162196e-02 -
                           3.61991396354483e-06 * max(pressure, 0.0) +
                           7.97204102509724e-12 * pow(max(pressure, 0.0), 2.)) *
                              max(variables[1], 0.0) +
                          (-7.53557031774437e-03 +
                           6.32712093275576e-08 * max(pressure, 0.0) -
                           1.66203631393248e-13 * pow(max(pressure, 0.0), 2.)) *
                              pow(max(variables[1], 0.0), 2.) +
                          (4.60380647957350e-05 -
                           5.61299059722121e-10 * max(pressure, 0.0) +
                           1.80924436489400e-15 * pow(max(pressure, 0.0), 2.)) *
                              pow(max(variables[1], 0.0), 3.) +
                          (-2.26651454175013e-07 +
                           3.36874416675978e-12 * max(pressure, 0.0) -
                           1.30352149261326e-17 * pow(max(pressure, 0.0), 2.)) *
                              pow(max(variables[1], 0.0), 4.) +
                          (6.14889851856743e-10 -
                           1.06165223196756e-14 * max(pressure, 0.0) +
                           4.75014903737416e-20 * pow(max(pressure, 0.0), 2.)) *
                              pow(max(variables[1], 0.0), 5.) +
                          (-7.39221950969522e-13 +
                           1.42790422913922e-17 * max(pressure, 0.0) -
            ...
            {
                const double M0 =
                    cp_vec[0]->molar_mass;  // molar mass of component 0
                const double M1 = cp_vec[1]->molar_mass;
                const double p = variables[0];
                const double T = variables[1];
                const double x =
                    variables[2];  // gas mass fraction of component 1
                // assert(0.0 <= x && x <= 1.0);

                // gas molar fraction of component 1
                const double xn = M0 * x / (M0 * x + M1 * (1.0 - x));

                density = p / (PhysicalConstant::IdealGasConstant * T) *
                          (M1 * xn + M0 * (1.0 - xn));  // R_uni in mNs
            }
            break;
    ...
    else
    {
        ...
        switch (density_model)
        {
            ...
            case 10:  // Get density from temperature-pressure values from
                      // fct-file NB
                density =
                    GetMatrixValue(primary_variable[1], primary_variable[0],
                                   fluid_name, &gueltig);
                break;
            ...
            case 18:  // using calculated densities at nodes from the phase
                ...
                std::cout << " Error - Density Model 18 not implemented, usind "
                             "dummy density of 1000."
                          << "\n";
            ...
            case 19:  // KG44 get the density from GEMS calculations
                ...
                {
                    long elem = Fem_Ele_Std->GetMeshElement()
                                    ->GetIndex();  // kg44 need element index or
                                                   // node index for
                    // GEMS ...currently we stick to arithmetric
                    // average of node data (even for gauss
                    // points!)
                    density = m_vec_GEM->REACT_GEM::FluidDensity(
                        elem, -1);  // hand over element index and set gauss
                                    // point to -1...
                    // Remark: Interpolation from Gauss points to element is not
                    // possible here: Fluid densities are only calculated at
                    // nodes, therefore interpolation from gauss points onto
            ...
            case 20:  // rho(p,T, C) for water, range p < 100 MPa, 0 <= T <= 350
                ...
                if (fabs(drho_dC) > 1.e-20)
                    density *=
                        1. + drho_dC * (max(primary_variable[2], 0.0) - C_0);
                break;
            case 26:  // Dalton's law + ideal gas for use with TNEQ/TES
            {
                const double M0 =
                ...
                const double T = primary_variable[1];
                const double x =
                    primary_variable[2];  // gas mass fraction of component 1
                // assert(0.0 <= x && x <= 1.0);
            ...
            default:
                std::cout << "Error in CFluidProperties::Density : no valid "
                             "model (variables definded?)"
                          << "\n";
                break;
        }
    }
...
/*-------------------------------------------------------------------------
   ...
   GPIndex < 0) or to element gauss points Return: interpolated variable
   Programming: 11/2010 BG
   Modification:
   -------------------------------------------------------------------------*/
double CFluidProperties::GetElementValueFromNodes(long ElementIndex,
                                                  int GPIndex, int PhaseIndex,
                                                  int VariableIndex)
{
    ...
    if ((density_model == 18) || (viscosity_model == 18))
        if (m_pcs->Phase_Transition_Model != 1)
        {
            cout << "The Phase_Transition_Model should be used together with "
                    "the density and viscosity model 18 !"
                 << "\n";
    ...
    if (GPIndex > -1)
    {
        ...
        if (m_ele->GetMark())  // Marked for use
            ...
            // velocities Fem_Ele_Std->ConfigElement(m_ele);
...
double CFluidProperties::Viscosity(double* variables)
{
    ...
    switch (viscosity_model)
    {
        ...
        {
            int gueltig;
            viscosity = GetCurveValue(viscosity_curve_number, 0,
                                      primary_variable[1], &gueltig);
            break;
        }
        case 1:  // my = const
            viscosity = my_0;
        ...
        case 9:  // viscosity as function of density and temperature, NB
        {
            ...
            mfp_arguments[0] =
                primary_variable[0];  // rescue primary_variable before its
                                      // destroyed by Density();
        ...
        case 15:  // mixture 1/�= sum_i y_i/�_i:: VTPR-EoS
        {
            ...
            for (int CIndex = 2; CIndex < cmpN + 2; CIndex++)
            {
                ...
                else
                {
                    ...
                    my += variables[CIndex] /
                          Fluid_Viscosity(ComponentDensity(CIndex, variables),
                                          variables[1], variables[0], fluid_id);
                }
            }
            viscosity = 1.0 / my;
            break;
        ...
        case 19:  // reserved for GEMS coupling
            break;
        case 26:  // Wilke (see Poling, B. E.; Prausnitz, J. M.; John Paul, O. &
                  // Reid, R. C. The properties of gases and
            // liquids McGraw-Hill New York, 2001, 5: page 9.21)
            {
                ...
                // reactive component
                x[0] = M1 * X /
                       (M1 * X + M2 * (1.0 - X));  // mass in mole fraction
                Vs[0] = Fluid_Viscosity(
                    M2 * p / (PhysicalConstant::IdealGasConstant * T), T, p,
                    cp_vec[1]->fluid_id);
                // inert component
                x[1] = 1.0 - x[0];
                Vs[1] = Fluid_Viscosity(
                    M1 * p / (PhysicalConstant::IdealGasConstant * T), T, p,
...
double CFluidProperties::LiquidViscosity_Yaws_1976(double T)
{
    ...
    // (1976) B =  1.9490e+03; C =  2.1641e-02; D = -1.5990e-05;
...
double CFluidProperties::PhaseDiffusion(double* variables)
{
    ...
    {
        primary_variable[0] =
            variables[0];  // p (single phase)// SP: has to be T
        primary_variable[1] =
            variables[1];  // T (temperature) // SP: has to be p
        primary_variable[2] =
            variables[2];  // C (salinity)//index in case of AIR_FLOW
    }
    else
    ...
    switch (diffusion_model)
    {
        ...
        case 2:  // my(p) = my_0*(1+gamma_p*(p-p_0))
            ...
            break;
        default:
            cout << "Error in CFluidProperties::PhaseDiffusion: no valid model"
                 << endl;
            break;
    }
    //----------------------------------------------------------------------
...
double CFluidProperties::SpecificHeatCapacity(double* variables)
{
    ...
    switch (heat_capacity_model)
    {
        ...
        case 9:
            ...
            break;
        case 11:  // mole fraction weighted average of molar isochoric specific
                  // heat capacities converted into isobaric
            // mixture specific heat capacity (ideal conversion)
            {
                ...
                Cp_c[0] = isochoric_heat_capacity(M2 * p / (R * T), T,
                                                  cp_vec[1]->fluid_id);
                // inert component
                x[1] = 1.0 - x[0];
                therm_prop("N");
                Cp_c[1] = isochoric_heat_capacity(M1 * p / (R * T), T,
                                                  cp_vec[0]->fluid_id);
                specific_heat_capacity =
                    Cp_c[0] * M2 * x[0] +
                    Cp_c[1] * M1 *
                        x[1];  // mixture isochoric molar heat capacities
                specific_heat_capacity += R;  // isochoric in isobaric
                specific_heat_capacity /=
                    (M1 * x[1] +
                     M2 * x[0]);  // molar in specific of mixture value
                break;
            }
        case 12:  // mass fraction weighted average of isobaric specific heat
                  // capacities using a linearised model
            // reactive component
            ...
            specific_heat_capacity =
                Cp_c[0] * x[0] +
                Cp_c[1] * x[1];  // mixture isobaric specific heat capacities
            break;
        case 13:  // mass fraction weighted average of isobaric specific heat
                  // capacities using a polynomial model
            // reactive component
            x[0] = primary_variable[2];  // mass fraction
            Cp_c[0] =
                polynomial_heat_capacity(variables[1], cp_vec[1]->fluid_id);
            // inert component
            x[1] = 1.0 - x[0];
            Cp_c[1] =
                polynomial_heat_capacity(variables[1], cp_vec[0]->fluid_id);
            specific_heat_capacity =
                Cp_c[0] * x[0] +
                Cp_c[1] * x[1];  // mixture isobaric specific heat capacities
            break;
                ...
                else
                {
                    ...
                    Cp += variables[CIndex] *
                          isobaric_heat_capacity(
                              ComponentDensity(CIndex, variables), variables[1],
                              fluid_id);
                }
            }
...
double CFluidProperties::PhaseChange()
{
    ...
    else if (heat_capacity_model == 4)
    {
        ...
        {
            humi = exp(pressure /
                       (PhysicalConstant::SpecificGasConstant::WaterVapour *
                        temperature_buffer * Density()));
            density_vapor = humi * Density();
            drdT =
                (vaporDensity_derivative(temperature_buffer) * humi -
                 density_vapor * pressure /
                     (PhysicalConstant::SpecificGasConstant::WaterVapour *
                      Density() * (temperature_buffer * temperature_buffer))) /
                Density();
            H1 = latent_heat +
...
double CFluidProperties::HeatConductivity(double* variables)
{
    ...
    switch (heat_conductivity_model)
    {
        ...
        case 11:  // Wassilijewa, Maso&Saxena (see Poling, B. E.; Prausnitz, J.
                  // M.; John Paul, O. & Reid, R. C. The
            // properties of gases and liquids McGraw-Hill New York, 2001, 5:
            // page 10.30f.)
            {
                double x[2], k[2];
                ...
                // TODO [CL] max() is redundant if the fraction is guaranteed to
                // be between 0 and 1. reactive component
                const double R = PhysicalConstant::IdealGasConstant;
                x[0] = max(M0 * X / (M0 * X + M1 * (1.0 - X)),
                           0.);  // convert mass to mole fraction
                k[0] = Fluid_Heat_Conductivity(M1 * p / (R * T), T,
                                               cp_vec[1]->fluid_id);
                // inert component
                x[1] = 1.0 - x[0];
                k[1] = Fluid_Heat_Conductivity(M0 * p / (R * T), T,
                                               cp_vec[0]->fluid_id);

                const double M1_over_M2 = M1 / M0;  // reactive over inert
                const double V1_over_V2 =
                    Fluid_Viscosity(M1 * p / (R * T), T, p,
                                    cp_vec[1]->fluid_id) /
                    Fluid_Viscosity(M0 * p / (R * T), T, p,
                                    cp_vec[0]->fluid_id);
                const double L1_over_L2 = V1_over_V2 / M1_over_M2;

                const double phi_12 =
                    (1.0 + pow(L1_over_L2, 0.5) * pow(M1_over_M2, -0.25)) *
                ...
                else
                {
                    ...
                    Kappa += variables[CIndex] *
                             Fluid_Heat_Conductivity(
                                 ComponentDensity(CIndex, variables),
                                 variables[1], fluid_id);
                }
            }
                                              ...
                                              double C)
{
    ...
    double my_Zero, PsatBar,
        PsatKPa; /*my pure water, my saline water, Saturation pressure [bar],
                                                        ...
                                                        double Conc)
{
    ...
    GammaPiPi = 0;
    for (i = 1; i <= 34; i++)
        GammaPiPi = GammaPiPi + (nGamma[i]) * (LGamma[i]) * (LGamma[i] - 1) *
                                                         ...
                                                         double Conc)
{
    ...
    for (i = 1; i < 35; i++)
        GammaTauTau =
            GammaTauTau + (n[i]) * (pow((7.1 - Pressurevar), (L[i]))) * (J[i]) *
    ...
    GammaPiPi = 0;
    for (i = 1; i < 35; i++)
        GammaPiPi = GammaPiPi + (n[i]) * (L[i]) * (L[i] - 1) *
                       ...
                       int phase_number, const bool for_output)
{
    ...
    for (unsigned i = 0; i < vec_var_names->size(); i++)
    {
        ...
        if (pcs)
        {
            ...
            else if ((*vec_var_names)[i] == "TEMPERATURE1")
            {
                ...
                std::cout << "The variable " << (*vec_var_names)[i]
                          << " is not supported in MFPGetNodeValue."
                          << std::endl;
        }
        else
        {
    ...
    if (m_mfp->cmpN > 0)
    {
        ...
        for (int PVIndex = 0; PVIndex < m_mfp->cmpN + 2; PVIndex++)
            arguments[PVIndex] = m_pcs->GetNodeValue(
                node, m_pcs->GetNodeValueIndex(
                          m_pcs->pcs_primary_function_name[PVIndex]));
    }
...
fraction Programing: 09/2009 NB
**************************************************************************/
    ...
    switch (compressibility_model_pressure)
    {
        ...
        case 15:  // volume translated Peng-Robinson
            if (eos_name == "VTPR" || eos_name == "PR" || eos_name == "IDEAL")
            {
                ...
                for (int CIndex = 2; CIndex < cmpN + 2; CIndex++)
                {
                    double A, B, c, C, dvdp, fctA, fctB, fctC, beta, Tr, z, z1,
                        z2, z3, v, Trr, fct;
                    therm_prop(m_pcs->pcs_primary_function_name[CIndex]);
                    Tr = T / Tc;
                    // double a0 = 1.0 + m0*(1.0 - Tr) + n0*(1.0 - Tr)*(0.7 -
                    // Tr);
                    ...
                    C = c * p * pow(R * T, -1.0);
                    z1 = B - 1.0 + 3.0 * C;
                    z2 = (A - 3.0 * B * B + 3.0 * C * C + 2.0 * B * C -
                          2.0 * B - 2.0 * C);
                    z3 = (B * B * B + C * C * C + B * B - C * C + B * C * C -
                          3.0 * B * B * C - 2.0 * B * C + C * A - A * B);
...
mass fraction Programing: 09/2009 NB
**************************************************************************/
double CFluidProperties::drhodT(double* variables)
{
    double a0, A, B, c, C, da0, da, alpha, alpha_m, dvdT, fctA, fctB, fctC, p,
        R, T, Tr, z, z1, z2, z3, v, v_m, Trr, fct;
    std::vector<double> roots;
    CRFProcess* m_pcs;
    m_pcs = PCSGet("MULTI_COMPONENTIAL_FLOW");
    p = variables[0];
    ...
    switch (compressibility_model_temperature)
    {
        ...
        case 15:  // volume translated Peng-Robinson
            if (eos_name == "VTPR" || eos_name == "PR" || eos_name == "IDEAL")
            {
                ...
                drhodT =
                    -alpha_m / v_m;  // alpha_t=alpha_g*(vg/vt)+alpha_l*(vl/vt)
            }
    ...
    switch (solutal_expansivity_model)
    {
        ...
        case 15:
            if (eos_name == "CONSTANT")
                ...
                drhodX =
                    -Density(variables) / ComponentDensity(CIndex, variables);
            break;
