...
// Solve for velocity
            ...
            for (int j = 0; j < size; j++)
                m_pcs->SetNodeValue(
                    m_msh->Eqs2Global_NodeIndex[j], nidx1, x[j]);
            ...
            for (int j = 0; j < eqs->dim; j++)
                m_pcs->SetNodeValue(
                    m_msh->Eqs2Global_NodeIndex[j], nidx1, eqs->x[j]);
#endif
        }
        ...
        /*
           ...
           for(i = 0; i < (long)m_msh->nod_vector.size(); i++)
           {
              ...
              double norm[3];
              if(m_msh->GetCoordinateFlag() != 32 && m_msh->GetCoordinateFlag()
           != 22)
           {
           ...
           // Do some proper projection of velocity computed from Fluid
           Momentum.
           // Get the fluid velocity for this node
           double V[3];
           V[0] = m_pcs->GetNodeValue(i,
           m_pcs->GetNodeValueIndex("VELOCITY1_X")+1); V[1] =
           m_pcs->GetNodeValue(i, m_pcs->GetNodeValueIndex("VELOCITY1_Y")+1);
           V[2] = m_pcs->GetNodeValue(i,
        ...
        for (i = 0; i < (long)m_msh->ele_vector.size(); i++)
        {
            ...
            /*
               ...
               m_pcs->GetElementValueIndex("VELOCITY2_Y")+1, vy);
                                m_pcs->SetElementValue(i,
               m_pcs->GetElementValueIndex("VELOCITY2_Z")+1, vz); break;
                        default:
...
void CFluidMomentum::ConstructFractureNetworkTopology()
{
    ...
    for (int i = 0; i < (int)m_msh->nod_vector.size(); ++i)
    {
        ...
        for (int j = 1; j < NumOfNeighborElements; ++j)
        {
            ...
            else
            {
                ...
                // different from the reference element. Then, I will get rid of
                // the duplicate elements of the same plane.
                int indexOther = -1;
        ...
        // potential crossroads This works great with h_frac example in RWPT
        ...
        if (thisNode->crossroad == false)
        {
            ...
            // Store the projected velocity back to the node velocity
            m_pcs->SetNodeValue(
                i, m_pcs->GetNodeValueIndex("VELOCITY1_X") + 1, Vp[0]);
            m_pcs->SetNodeValue(
                i, m_pcs->GetNodeValueIndex("VELOCITY1_Y") + 1, Vp[1]);
            m_pcs->SetNodeValue(
                i, m_pcs->GetNodeValueIndex("VELOCITY1_Z") + 1, Vp[2]);
        }
        else
        {
            ...
            // of the center of the connected elements. Now some dot product.
            for (int j = 0; j < thisCross->numOfThePlanes; ++j)
            {
                double angle = 0.0;
                double a[3], b[3];
    ...
    for (int i = 0; i < (int)m_msh->edge_vector.size(); ++i)
    {
            ...
            theNodesOfThisEdge[1]->crossroad)
        {
            ...
            // the center of the connected elements. Now some dot product.
            for (int j = 0; j < thisJoint->numOfThePlanes; ++j)
            {
                double angle = 0.0;
                double a[3], b[3];
...
void DATWriteHETFile(const char* file_name)
{
    ...
    CFEMesh* m_msh = NULL;
    m_msh = fem_msh_vector[0];            // Something must be done later on
here. MeshLib::CElem* elem = NULL;
