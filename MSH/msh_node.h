...
public:
    ...
    // on. Establishing this vector is done in the Fluid Momentum since this is
    // bounded by velocity.
    ...
    bool crossroad;  // KR changed to bool // PCH: Make theses privates can be
                     // done later on.
    std::vector<long>
        connected_faces;  // BG, 09/2010, necessary for coupling to Eclipse,
                          // index of faces where the node is part of it
     ...
     * \param id_max_act_l Maximum node ID of the non-ghost node for linear
     * element. \param id_max_l     Maximum node ID of the node of linear
     * element. \param id_max_act_h Maximum node ID of the non-ghost node for
     * quadratic element.
     */
    bool isNonGhost(const size_t id_max_act_l, const size_t id_max_l,
                    const size_t id_max_act_h)
    {
        return (index < id_max_act_l) ||
               (index >= id_max_l && index < id_max_act_h);
    }
