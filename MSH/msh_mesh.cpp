...
void CFEMesh::computeSearchLength(double c)
{
    ...
    if (n == 0)
    {
        std::cerr << "[CFEMesh::computeSearchLength] no edges found for "
                     "computing _search_length, setting _search_lenght to "
                  << 1e-3 << "\n";
        _search_length = 1e-3;
        return;
    }
    ...
    else
    {
        ...
        {
            std::cerr
                << "[CFEMesh::computeSearchLength] computed _search_length = "
                << _search_length << ", the average value is: " << mu
...
bool CFEMesh::Read(std::ifstream* fem_file)
{
    ...
    while (!fem_file->eof())
    {
        ...
        if (line_string.find("#FEM_MSH") !=
            std::string::npos)  // 12.08.2011. WW
        {
            more_mesh = true;
            break;
        }
...
void CFEMesh::ConstructGrid()
{
...
#if !defined( \
    USE_PETSC)  // &&! defined(USE_OTHER Parallel solver lib) //WW 06.2013
    ...
    for (size_t e = 0; e < e_size; e++)
    {
        ...
        for (size_t i = 0; i < nedges0; i++)  // edges
        {
            ...
            {
                size_t nConnElem(e_nodes0[edgeIndex_loc0[k]]
                                     ->getConnectedElementIDs()
                                     .size());
                         ...
                         ii++)  // edges of element connected to edge node
                    {
                        ...
                        if ((node_index[edgeIndex_loc0[0]] ==
                            ...
                            (node_index[edgeIndex_loc0[0]] ==
                             ...
                             node_index[edgeIndex_loc0[1]] ==
                                 node_index_glb
                                     [edgeIndex_loc[0]]))  // check if elements
                                                           // share edge
    ...
    else if (xyz_dim[0] > 0.0 && xyz_dim[1] > 0.0 &&
             xyz_dim[2] > 0.0)  // x, y & z direction
                          ...
                          double search_radius)
{
    ...
    else
    {
        ...
        const size_t end_act_node_h =
            NodesNumber_Linear + static_cast<size_t>(loc_NodesNumber_Quadratic -
                          ...
                          ) const
{
    ...
    std::cout << "[CFEMesh::GetNODOnSFC] search with new algorithm ... "
              << std::flush;
    begin = clock();
#endif
#if defined(USE_PETSC)  // || defined (other parallel linear solver lib). //WW.
    ...
    std::cout << "done, took " << (end - begin) / (double)(CLOCKS_PER_SEC)
              << " s, " << msh_nod_vector.size() << "nodes found"
              << "\n";
#endif
}
                                 ...
                                 std::vector<long>& node_id_vector) const
{
    ...
    for (size_t j = start_id; j < end_id; j++)
    {
        ...
        for (size_t i = 0; i < np; i++)
        {
            ...
            area_calculated +=
                fabs(ComputeDetTri(x1, nod_vector[j]->getData(), x2));
        }
...
// Check nodes by comparing area
        ...
        if (for_s_term)
        {
            findNodesInPolygon(Area1, Tol, 0, NodesInUsage(), m_ply,
                               msh_nod_vector);
        }
        else
        {
            findNodesInPolygon(Area1, Tol, 0, getNumNodesLocal(), m_ply,
                               msh_nod_vector);

            if (useQuadratic)
            {
                findNodesInPolygon(Area1, Tol, GetNodesNumber(false),
                                   getLargestActiveNodeID_Quadratic(), m_ply,
                                   msh_nod_vector);
            }
        }
#else
    ...
    while (anz_relevant == 0)
    {
        ...
        for (j = 0; j < (long)nod_vector.size(); j++)
        {
            ...
            for (k = 0; k < (long)m_ply->point_vector.size() - 1; k++)
            {
                ...
                // MCalcDistancePointToLine, MCalcDistancePointToPoint and
                // MCalcDistancePointToPoint have no side effects
                pt1[0] = pnt_j[0];
                pt1[1] = pnt_j[1];
                pt1[2] = 0.0;
                line1[0] = m_ply->point_vector[k]->x;
        ...
        for (size_t j = 0; j < n_edges; j++)
        {
            ...
            for (size_t k = 0; k < nodes_near_ply.size(); k++)
            {
                ...
                if (elem->GetNodeIndex(loc_edge_nidx[0]) ==
                    static_cast<long>(nodes_near_ply[k]))
                    selected++;
...
size_t CFEMesh::FindElementByPoint(const double* xyz)
{
    ...
    for (size_t i = 0; i < ne; i++)
    {
        ...
        if (n_ele->GetElementType() == MshElemType::QUAD ||
            n_ele->GetElementType() == MshElemType::TETRAHEDRON)
        {
        ...
        switch (n_ele->GetElementType())
        {
            ...
            case MshElemType::QUAD:
                ...
                if (fabs((a_sub[0] + a_sub[1] + a_sub[2] + a_sub[3] - a) / a) <
                    tol)
                {
                    return i;
                }
            ...
            case MshElemType::PYRAMID:
                ...
                if (fabs((a_sub[0] + a_sub[1] + a_sub[2] + a_sub[3] + a_sub[4] +
                          a_sub[5] - a) /
                         a) < tol)
                {
                    return i;
            ...
            case MshElemType::HEXAHEDRON:
                ...
                if (fabs((a_sub[0] + a_sub[1] + a_sub[2] + a_sub[3] + a_sub[4] +
                          a_sub[5] + a_sub[6] + a_sub[7] + a_sub[8] + a_sub[9] +
                          a_sub[10] + a_sub[11] - a) /
                         a) < tol)
                {
                    return i;
...
last modification:
**************************************************************************/
void CFEMesh::GetConnectedElements(std::vector<long>& nodes_on_sfc,
                                   std::vector<long>& vec_elements)
{
    ...
    {
        k = nodes_on_sfc[i];
        for (j = 0; j < (long)nod_vector[k]->getConnectedElementIDs().size();
             j++)
        {
...
void CFEMesh::markTopSurfaceFaceElements3D()
{
    ...
    for (std::size_t i = 0; i < face_vector.size(); i++)
    {
        ...
        Math_Group::Matrix const* const transform_tensor =
            elem->getTransformTensor();
        double fac = cent[0] * (*transform_tensor)(0, 2) +
                     cent[1] * (*transform_tensor)(1, 2) +
