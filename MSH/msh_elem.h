...
public:
    ...
    ///  \param recompute_matrix, an indicator to determine whether the
    ///  transform matrix
    ///                           can be recomputed. For face integration
    ///                           calulation in
    ///                           CSourceTerm::FaceIntegration(...), only one
    ///                           instance of this class is created for all
    ///                           found face elements, therefore
    ///                           recompute_matrix must be set to true. If each
    ///                           geometry element gets an intance of this
    ///                           class, recompute_matrix has to be set to
    ///                           false.
    ...
    Math_Group::Matrix* getTransformTensor() const { return transform_tensor; }

    void AllocateMeomoryforAngle() { angle.resize(3); }       // WW
    ...
    {
        return excavated;
    }  // WX:01.2011 get excavation state
